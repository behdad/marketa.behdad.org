...
The self-hosted Python runtime behind rsvp.html's python app: Pyodide v314.0.2
...
Core runtime (what `loadPyodide({ indexURL: "art/pyodide/" })` fetches):
...
- `pyodide-lock.json` — the release's package index; micropip resolves any package
  listed here from this directory instead of the network (114 KB)
...
here and the `~15 MB` figure in rsvp.html's `py_loading` copy (EN + CS) if the
