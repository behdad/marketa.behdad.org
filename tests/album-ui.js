...
var HARNESS = [
  ...
  ' window.__loftControllers.phone.open("album");await sleep(120);',
  ...
  ' window.__loftControllers.phone.open("album");await sleep(80);card=document.querySelector(".pm-pol-selfie");var del=card&&card.querySelector(".pm-pol-delete");if(del)del.click();await sleep(80);S("remove",{count:window.__albumList().length,selfie:!!document.querySelector(".pm-pol-selfie"),stillAlbum:!!document.querySelector(".pm-album")});',
  ' window.__setSecondRound(true,{releaseHeld:false});window.__setGardenParty(true,false);var portrait=window.__albumAdd(true);var group=window.__albumAddGroupPhoto();window.__setGardenParty(false,true);window.__loftControllers.phone.open("album");await sleep(80);',
  ' var recap=document.querySelector(".pm-album-recap-toggle");var liveCount=window.__albumList().filter(function(x){return !x.shoot;}).length;await sleep(1100);if(recap)recap.click();await sleep(80);var headings=Array.prototype.map.call(document.querySelectorAll(".pm-album-section h3"),function(x){return x.textContent;});S("recap",{available:!!recap,portrait:!!portrait,group:!!group,open:document.querySelector(".pm-album-grid").classList.contains("recap"),sections:headings,cards:document.querySelectorAll(".pm-album-section-grid .pm-polaroid").length,live:liveCount});',
  ' window.__setLang("cs");await sleep(80);S("recapCs",{title:document.querySelector(".pm-album-recap b").textContent,headings:Array.prototype.map.call(document.querySelectorAll(".pm-album-section h3"),function(x){return x.textContent;})});',
  '}',
  '})();</script>'
].join("\n");
