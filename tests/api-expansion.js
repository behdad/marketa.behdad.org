...
var HARNESS = [
  ...
  ' if(window.__dismissActivePartyCoach)window.__dismissActivePartyCoach();await new Promise(function(resolve){setTimeout(resolve,120);});window.__goToStage("office");await new Promise(function(resolve){setTimeout(resolve,300);});var monitor=document.getElementById("office-monitor");if(window.__loftControllers.computer&&window.__loftControllers.computer.status())window.__loftControllers.computer.set(false);await new Promise(function(resolve){setTimeout(resolve,120);});',
  ' var lifeOpen=await api.perform("app.open",{app:"life"},{source:"test"});await ready(function(){var current=api.query("app.current");return current.ok&&current.value.monitor==="life"&&window.__monitorAppRunning&&window.__monitorAppRunning("life");},"Life",4000);var lifeBefore=api.query("app.current"),killStart=performance.now(),lifeKill=await api.perform("app.kill",{app:"life",device:"monitor"},{source:"test"}),lifeAfter=api.query("app.current");',
