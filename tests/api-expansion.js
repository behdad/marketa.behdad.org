...
check(q.info.ok && q.info.value.version === 2 && Number.isInteger(q.info.value.stateVersion) && q.caps.ok && q.caps.value.every(function (cap) { return cap.kind === "query"; }), "API self-description reports version and filtered capabilities", { info: q.info, count: q.caps && q.caps.value && q.caps.value.length });
...
check(q.media.ok && typeof q.media.value.master_volume === "number" && q.environment.ok && typeof q.environment.value.daylight === "boolean" && q.game.ok && q.game.value.busy && Array.isArray(q.game.value.unlocked_rooms), "combined media, environment, and game status stay structured", { media: q.media, environment: q.environment, game: q.game });
...
check(v.typedParty.delta === 1 && v.typedParty.events.length === 1 && v.typedParty.events[0].id === "party.set" && v.typedParty.events[0].source === "test", "composite typed actions coalesce central mutations into one revision", v.typedParty);
