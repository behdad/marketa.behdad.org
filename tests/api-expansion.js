...
check(q.info.ok && q.info.value.version === 4 && Number.isInteger(q.info.value.stateVersion) && q.caps.ok && q.caps.value.every(function (cap) { return cap.kind === "query" && typeof cap.group === "string"; }), "API self-description reports version, groups, and filtered capabilities", { info: q.info, count: q.caps && q.caps.value && q.caps.value.length });
