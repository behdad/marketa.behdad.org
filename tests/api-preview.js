...
var HARNESS = [
  ...
  'async function run(){',
  ' var raw="{ \\"byte\\": \\"for byte\\", \\"spacing\\": true }",key="loftCheckpoint:v1";localStorage.setItem(key,raw);localStorage.setItem("preview-test","before");window.__secondRound=true;',
  ...
  ' S("preview",{raw:raw,before:before,begin1:begin1,begin2:begin2,live:live,room:room,finite:finite,suppressed:{album:album,message:message,external:external,destructive:destructive},partyImmediate:partyImmediate,route:route,road:road,ended:ended,after:after,beginFresh:beginFresh,reset:reset,resetRoom:resetRoom,fresh:fresh,freshAfter:freshAfter});',
  '}',
  '})();</script>'
].join("\n");
