...
var HARNESS = [
  ...
  ' Object.keys(TRIP_DURATIONS).forEach(function(key){TRIP_DURATIONS[key]=120;});window.__secondRound=false;var tripPhaseBefore=window.__secondRound,tripRoomBefore=window.currentStageName;v=begin();var tripNextResult=await api.perform("trip.next",{},{source:"test"});await wait(30);var tripNext=finish(v),tripState=api.query("trip.status"),tripRoomAfter=window.currentStageName;',
