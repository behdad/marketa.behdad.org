...
var HARNESS = [
  ...
  ' window.__maxUnlocked=oldMax;off();S("state",{version:api.version,projectorDirect:projectorDirect,projectorNoop:projectorNoop,projectorTyped:projectorTyped,projectorTypedResult:projectorTypedResult,weatherDirect:weatherDirect,weatherNoop:weatherNoop,weatherTyped:weatherTyped,weatherTypedResult:weatherTypedResult,bbqDirect:bbqDirect,bbqTyped:bbqTyped,bbqTypedResult:bbqTypedResult,bbqNoop:bbqNoop,bbqNoopResult:bbqNoopResult,gameStart:gameStart,gameStop:gameStop,gameStopResult:gameStopResult,albumAdd:albumAdd,albumRemove:albumRemove,albumRemoveResult:albumRemoveResult,albumNoop:albumNoop,albumRemoveAgain:albumRemoveAgain,rang:rang,callRing:callRing,callDismiss:callDismiss,appOpen:appOpen,appNavigate:appNavigate,appNoop:appNoop,appNoopResult:appNoopResult,appClose:appClose,phoneCallAnswer:phoneCallAnswer,phoneCallEnd:phoneCallEnd,delayedHangupResult:delayedHangupResult,delayedHangupImmediate:delayedHangupImmediate,delayedHangupDone:delayedHangupDone,musicPlay:musicPlay,musicPlayResult:musicPlayResult,musicPause:musicPause,musicPauseResult:musicPauseResult,videoPlay:videoPlay,videoPause:videoPause,videoPauseResult:videoPauseResult,videoNoop:videoNoop,videoPauseAgain:videoPauseAgain,media:api.query("media.status"),calls:api.query("calls.status"),games:api.query("minigames.status")});',
  '}',
  '})();</script>'
].join("\n");
...
check(s.rang && one(s.callRing, "calls.changed", "autonomous") && s.callRing.last.args.incoming, "incoming calls publish their actual ringing lifecycle", s.callRing);
...
check(one(s.delayedHangupDone, "calls.changed", "autonomous") && !s.delayedHangupDone.last.args.busy, "a delayed hangup publishes exactly once when the call actually ends", s.delayedHangupDone);
