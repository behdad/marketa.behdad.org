...
var HARNESS = [
  ...
  ' v=begin();var tripNextResult=await api.perform("trip.next",{},{source:"test"});await wait(30);var tripNext=finish(v),tripState=api.query("trip.status");',
...
check(s.tripNext.events.filter(function(event){return event.id === "trip.next" && event.source === "test";}).length === 1 && s.tripNextResult.ok && s.tripNextResult.changed && s.tripNextResult.value.room === "garden" && s.tripState.ok && s.tripState.value.active && s.tripState.value.variant === s.tripNextResult.value.variant, "typed next-trip action follows the magic box's shuffled sequence and reports the real variant", { step: s.tripNext, result: s.tripNextResult, state: s.tripState });
