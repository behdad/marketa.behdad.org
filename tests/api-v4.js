...
var HARNESS = [
  ...
  ' var help={all:window.help(),loft:window.help(window.loft),api:window.help(window.loft.api),apiInfo:window.help(window.loft.api.info),kitchen:window.help(window.loft.kitchen),party:window.help(window.loft.party),weather:window.help(window.loft.weather),rain:window.help(window.loft.weather.rain),rainSet:window.help(window.loft.weather.rain.set),chest:window.help(window.loft.cuddly.chest),string:window.help("kitchen"),legacy:window.help("dance")};',
  ' var localeHelp={ok:false},oldLocaleCompare=String.prototype.localeCompare,oldCapabilities=api.capabilities;try{String.prototype.localeCompare=function(){throw new Error("typed help consulted localeCompare");};localeHelp.rain=window.help(window.loft.weather.rain);api.capabilities=function(options){return oldCapabilities.call(api,options).map(function(cap){var copy=Object.assign({},cap);if(cap.id==="api.capabilities")copy.group="Alpha";else if(cap.id==="api.info")copy.group="Zulu";return copy;});};localeHelp.groupTie=window.help();localeHelp.ok=localeHelp.rain===help.rain;}catch(error){localeHelp.error=String(error&&error.message||error);}finally{api.capabilities=oldCapabilities;String.prototype.localeCompare=oldLocaleCompare;}',
  ...
  ' S("api",{caps:caps,groups:groups,rooms:rooms,game:game,manifest:manifest,schemas:schemas,strict:strict,env:env,aliases:{bar:window.loft.bar===window.loft.kitchen,party:window.loft.party===window.loft.garden,roomBar:barRoom,roomParty:partyRoom,navBar:navBar,navParty:navParty},namespace:{partyImmediate:partyImmediate,pans:pans,chest:typeof window.loft.cuddly.chest.set,apiInfo:typeof window.loft.api.info,apiInfoResult:window.loft.api.info(),oldChest:typeof window.chest,oldPirate:typeof window.piratebox,oldStatus:typeof window.loftstatus},invalid:{auto:invalidAuto,gear:invalidGear},help:help,localeHelp:localeHelp,consoleHelp:consoleHelpText});',
...
check(["app.kill","app.python.status","caption.show","cuddly.chest.status","cuddly.chest.set","car.status","car.control.set","roadtrip.status","roadtrip.preview.show","camping.fire.place","camping.stew.select","camping.stargazing.trace","camping.finale.advance","session.preview.begin","session.preview.end","trailer.status","trailer.play","trailer.stop","presentation.reduced.status","presentation.card.show","presentation.card.hide","presentation.caption.show","presentation.chapter.show","presentation.cut","presentation.point","game.reset"].every(function (id) { return ids.includes(id); }), "room-owned controls, Python readiness, app Kill, bounded presentation, trailer, terminal act, and preview lifecycle are typed", ids.length);
check(s.schemas && s.schemas.environment.args.on.nullable && s.schemas.environment.argOrder.join(",") === "on" && s.schemas.environment.completion === "instant" && s.schemas.transmission.args.mode.enum.join(",") === "auto,manual" && s.schemas.route.args.route.enum.join(",") === "calgary,banff,abraham" && s.schemas.trip.completion === "finite" && s.schemas.fishu.completion === "finite" && s.schemas.activity.completion === "finite" && s.schemas.caption.completion === "instant" && s.schemas.caption.argOrder.join(",") === "text" && s.schemas.kill.completion === "finite" && s.schemas.kill.args.device.enum.join(",") === "current,phone,monitor" && s.schemas.presentation.args.card.enum.join(",") === "title,final" && !s.schemas.presentation.available, "argument order, nullable ownership, presentation gates, enums, and lifecycle completion are mechanically discoverable", s.schemas);
...
check(s.help && s.help.all === s.help.loft && /Loft typed API/.test(s.help.all) && /loft\.kitchen \(alias: loft\.bar\)/.test(s.help.all) && /loft\.garden \(alias: loft\.party\)/.test(s.help.all) && /loft\.weather/.test(s.help.all) && !/loft\.weather\.rain/.test(s.help.all) && !/\.set\(/.test(s.help.all) && !/legacy/i.test(s.help.all), "bare help and help(loft) stay concise at the top-level namespace boundary", s.help);
