...
var HARNESS = [
  ...
  ' var tub=document.getElementById("bathroom-tub"),drain=document.getElementById("bathroom-tub-drain-action"),bubbles=Array.from(room.querySelectorAll("[data-bath-bubble]")),bubbleClose=document.getElementById("bathroom-bubble-close");click(tub);var closeWaiting={pointer:getComputedStyle(bubbleClose).pointerEvents,state:window.__bathroomInteractionState()};await sleep(1200);var closeLive={pointer:getComputedStyle(bubbleClose).pointerEvents,transform:bubbleClose.getAttribute("transform"),ring:bubbleClose.querySelector(".game-close-ring").tagName.toLowerCase(),hit:bubbleClose.querySelector(".mini-hit").getAttribute("r")};click(bubbleClose);var closeState={room:window.__bathroomRoomState(),props:window.__bathroomInteractionState(),pointer:getComputedStyle(bubbleClose).pointerEvents};window.__resetBathroomProps();click(tub);var tubFilling=window.__bathroomInteractionState();await sleep(1200);var bubbleStart=window.__bathroomInteractionState();await sleep(300);var bubbleIdle=window.__bathroomInteractionState();bubbles.forEach(click);var bubbleDone=window.__bathroomInteractionState();click(tub);var bodyRepeat=window.__bathroomInteractionState();click(drain);var tubDraining=window.__bathroomInteractionState();await sleep(800);var bubbleReset=window.__bathroomInteractionState();report.steps.bubbles={count:bubbles.length,hits:bubbles.map(function(el){var hit=el.querySelector(".bathroom-bubble-hit");return [el.id,!!hit,hit&&getComputedStyle(hit).r];}),close:{waiting:closeWaiting,live:closeLive,stopped:closeState},filling:tubFilling,bodyRepeat:bodyRepeat,draining:tubDraining,start:bubbleStart.bubbles,idle:bubbleIdle.bubbles,done:bubbleDone.bubbles,reset:bubbleReset.bubbles};',
