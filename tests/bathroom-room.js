...
var HARNESS = [
  ...
  ' report.steps.down={state:window.__bathroomRoomState(),room:window.currentStageName,covered:window.__roomAmbienceCovered(),viewport:viewport.classList.contains("bathroom-room-open"),geometry:{room:[roomBox.left,roomBox.top,roomBox.width,roomBox.height],viewport:[viewBox.left,viewBox.top,viewBox.width,viewBox.height],kitchenBottom:kitchenBox.bottom,floor:{mark:floorButton.textContent,up:floorButton.classList.contains("floor-up"),aria:floorButton.hasAttribute("aria-label"),title:floorButton.hasAttribute("title"),gap:floorBox.left-dotsBox.right}},roster:[getComputedStyle(rosterToggle).visibility,getComputedStyle(roster).visibility,getComputedStyle(rosterBackdrop).visibility],messages:[getComputedStyle(badge).visibility,getComputedStyle(coach).visibility,getComputedStyle(thumb).visibility,getComputedStyle(call).visibility],images:room.querySelectorAll("img").length};',
  ...
  ' var tub=document.getElementById("bathroom-tub"),drain=document.getElementById("bathroom-tub-drain-action"),bubbles=Array.from(room.querySelectorAll("[data-bath-bubble]")),bubbleClose=document.getElementById("bathroom-bubble-close");click(tub);var closeLive={pointer:getComputedStyle(bubbleClose).pointerEvents,transform:bubbleClose.getAttribute("transform"),ring:bubbleClose.querySelector(".game-close-ring").tagName.toLowerCase(),hit:bubbleClose.querySelector(".mini-hit").getAttribute("r")};click(bubbleClose);var closeState={room:window.__bathroomRoomState(),props:window.__bathroomInteractionState(),pointer:getComputedStyle(bubbleClose).pointerEvents};window.__resetBathroomProps();click(tub);var tubFilling=window.__bathroomInteractionState();await sleep(1200);var bubbleStart=window.__bathroomInteractionState();bubbles.forEach(click);var bubbleDone=window.__bathroomInteractionState();click(tub);var bodyRepeat=window.__bathroomInteractionState();click(drain);var tubDraining=window.__bathroomInteractionState();await sleep(800);var bubbleReset=window.__bathroomInteractionState();report.steps.bubbles={count:bubbles.length,hits:bubbles.map(function(el){var hit=el.querySelector(".bathroom-bubble-hit");return [el.id,!!hit,hit&&getComputedStyle(hit).r];}),close:{live:closeLive,stopped:closeState},filling:tubFilling,bodyRepeat:bodyRepeat,draining:tubDraining,start:bubbleStart.bubbles,done:bubbleDone.bubbles,reset:bubbleReset.bubbles};',
...
var propNames = ["sink", "mirror", "tub-fill", "towel", "tub-drain", "stool", "cabinet", "scale", "toilet", "book"];
...
check(s.bubbles && s.bubbles.close && s.bubbles.close.live.pointer === "auto" &&
  s.bubbles.close.live.transform === "translate(194,15)" &&
