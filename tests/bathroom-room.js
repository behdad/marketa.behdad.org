...
var HARNESS = [
  ...
  ' var tub=document.getElementById("bathroom-tub"),bubbles=Array.from(room.querySelectorAll("[data-bath-bubble]"));click(tub);var tubFilling=window.__bathroomInteractionState();await sleep(1200);var bubbleStart=window.__bathroomInteractionState();bubbles.forEach(click);var bubbleDone=window.__bathroomInteractionState();click(tub);var bubbleReset=window.__bathroomInteractionState();report.steps.bubbles={count:bubbles.length,roles:bubbles.map(function(el){var hit=el.querySelector(".bathroom-bubble-hit");return [el.getAttribute("role"),el.getAttribute("tabindex"),el.getAttribute("aria-label"),!!hit,hit&&getComputedStyle(hit).r];}),filling:tubFilling,start:bubbleStart.bubbles,done:bubbleDone.bubbles,reset:bubbleReset.bubbles};',
  ' key("Enter");var enterStarting=window.__bathroomInteractionState();await sleep(1200);var enterOnce={filled:room.classList.contains("tub-filled"),state:window.__bathroomInteractionState()};key("Enter");var enterTwice={filled:room.classList.contains("tub-filled"),state:window.__bathroomInteractionState()};click(tub);report.steps.enterTub={starting:enterStarting,once:enterOnce,twice:enterTwice};',
...
["bathroom-sink", "bathroom-mirror-action", "bathroom-tub", "bathroom-waffle-towel",
 ...
 "bathroom-toilet-action", "bathroom-toilet-book", "bathroom-book-action", "bathroom-tub-shower",
