...
var HARNESS = [
  ...
  ' click(scale);click(tub);var mirrorFog=window.__bathroomInteractionState(),mirrorAction=document.getElementById("bathroom-mirror-action");click(mirrorAction);var mirrorReveal=window.__bathroomInteractionState();function mirrorClient(x,y){var p=art.createSVGPoint();p.x=x;p.y=y;return p.matrixTransform(art.getScreenCTM());}for(var di=0;di<5;di++){var a=mirrorClient(48+di*2,45),b=mirrorClient(63+di*2,58);mirrorAction.dispatchEvent(new PointerEvent("pointerdown",{bubbles:true,cancelable:true,pointerId:90+di,pointerType:"touch",button:0,buttons:1,clientX:a.x,clientY:a.y}));art.dispatchEvent(new PointerEvent("pointermove",{bubbles:true,cancelable:true,pointerId:90+di,pointerType:"touch",buttons:1,clientX:a.x,clientY:a.y}));art.dispatchEvent(new PointerEvent("pointermove",{bubbles:true,cancelable:true,pointerId:90+di,pointerType:"touch",buttons:1,clientX:b.x,clientY:b.y}));art.dispatchEvent(new PointerEvent("pointerup",{bubbles:true,cancelable:true,pointerId:90+di,pointerType:"touch",clientX:b.x,clientY:b.y}));}var mirrorDrawn={state:window.__bathroomInteractionState(),paths:document.querySelectorAll("#bathroom-mirror-doodle path").length,maxPoints:Math.max.apply(null,Array.from(document.querySelectorAll("#bathroom-mirror-doodle path")).map(function(path){return (path.getAttribute("d").match(/[ML]/g)||[]).length;}))};click(scale);var wetSpike={reading:document.getElementById("bathroom-scale-reading").textContent,state:window.__bathroomInteractionState()};setLang("cs");var wetCs=document.getElementById("bathroom-room-status").textContent,drawLabelCs=mirrorAction.getAttribute("aria-label");setLang("en");await sleep(380);var wetSettled=window.__bathroomInteractionState();click(scale);click(tub);var mirrorDrained=window.__bathroomInteractionState(),pathsDrained=document.querySelectorAll("#bathroom-mirror-doodle path").length;click(tub);var mirrorRefilled=window.__bathroomInteractionState();click(tub);click(scale);await sleep(380);report.steps.steam={fog:mirrorFog,reveal:mirrorReveal,drawn:mirrorDrawn,drained:mirrorDrained,pathsDrained:pathsDrained,refilled:mirrorRefilled,signature:document.getElementById("bathroom-mirror-signature").textContent,wetSpike:wetSpike,wetCs:wetCs,drawLabelCs:drawLabelCs,wetSettled:wetSettled,statusRole:document.getElementById("bathroom-room-status").getAttribute("role"),statusLive:document.getElementById("bathroom-room-status").getAttribute("aria-live")};',
...
check(s.steam && s.steam.fog.mirror.fogged && !s.steam.fog.mirror.revealed &&
  ...
  "wiping reveals the signature, touch doodles stay capped, and draining or refilling resets them",
  s.steam);
