...
check(s.cs && s.cs.room === "Ložnice" && s.cs.close === "Zpět do pracovny" &&
  ...
  s.cs.suits && s.cs.suits.length === 2 &&
  s.cs.suits.every(function (entry) { return entry[0] && entry[0] === entry[1]; }),
...
check(s.suits && /\bswinging\b/.test(s.suits.pink) && !/\bswinging\b/.test(s.suits.blue) &&
  ...
  s.suits.labels[0] === "Swing Behdad's pink wedding suit" &&
  s.suits.labels[1] === "Swing Markéta's blue wedding suit",
  "the open wardrobe reveals both labeled suits and each hanger swings independently", s.suits);
