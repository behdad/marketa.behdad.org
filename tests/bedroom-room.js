...
var HARNESS = [
  ...
  ' var burgundyShoes=document.getElementById("bedroom-shoes-burgundy"),blueShoes=document.getElementById("bedroom-shoes-blue");burgundyShoes.style.transition="none";blueShoes.style.transition="none";var burgundyArt=burgundyShoes.querySelector(".bedroom-shoe-art"),blueArt=blueShoes.querySelector(".bedroom-shoe-art"),bedroomArt=document.getElementById("bedroom-room-art"),shelfPoint=bedroomArt.createSVGPoint();shelfPoint.x=620;shelfPoint.y=234;var shelfY=shelfPoint.matrixTransform(bedroomArt.getScreenCTM()).y;click(burgundyShoes);report.steps.shoes={count:room.querySelectorAll(".bedroom-wedding-shoes").length,opacity:[getComputedStyle(burgundyShoes).opacity,getComputedStyle(blueShoes).opacity],burgundy:burgundyShoes.getAttribute("class"),blue:blueShoes.getAttribute("class"),suits:[box(pinkSuit),box(blueSuit)],art:[box(burgundyArt),box(blueArt)],shelfY:shelfY,paths:[burgundyArt.querySelectorAll("path").length,blueArt.querySelectorAll("path").length],blueDots:blueArt.querySelectorAll("circle").length};click(blueShoes);report.steps.shoes.blueAfter=blueShoes.getAttribute("class");',
...
check(s.shoes && s.shoes.count === 2 && s.shoes.opacity.every(function (opacity) { return opacity === "1"; }) &&
  ...
  "the open wardrobe seats the copied wedding shoes on the shelf beneath their matching suits and each pair shines independently",
  s.shoes);
