...
var HARNESS = [
  ...
  ' var room=document.getElementById("bedroom-room"),viewport=document.querySelector(".hunt-viewport"),strip=document.getElementById("loft-game-strip"),office=document.getElementById("stage-office");room.style.transition="none";viewport.style.transition="none";strip.style.transition="none";window.__goToStage("office");window.__setDayNight(false);await sleep(100);',
  ...
  ' window.__goToStage("office");dblclick(document.getElementById("office-stainedglass"));await sleep(40);report.steps.interactive=window.__bedroomRoomState();',
...
check(propState.length === 11 &&
  ...
  propHas("bedroom-bed", "duvet-flicker") &&
...
check(s.tableIsolation && !s.tableIsolation.bed && !s.tableIsolation.table,
  "a nightstand drawer click stays contained and does not replay the duvet flicker", s.tableIsolation);
...
check(s.return && !s.return.state.open && !s.return.state.closing && s.return.hidden &&
  ...
  s.return.state.props.every(function (prop) {
    ...
    return persistentLamp || !/\b(?:glinting|off|wiggling|tidied|duvet-flicker|open)\b/.test(prop.state);
  }),
