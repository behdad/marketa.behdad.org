...
var HARNESS = [
  ...
  ' var loftDateBefore=window.date();tap(date);await sleep(40);var generic=[].find.call(document.querySelectorAll(".calx-day"),function(x){var n=x.querySelector(".calx-num");return n&&n.textContent==="23";});if(generic)tap(generic);await sleep(260);S("calendarDay",{phone:!!document.querySelector(".phone-backdrop.show"),lock:lock.classList.contains("showing"),tens:document.getElementById("garden-boxlock-drum-t").style.transform,unit:document.getElementById("garden-boxlock-drum-u").style.transform,date:window.date(),before:loftDateBefore});',
  ...
  ' var openPrevented=ctx(box),picker=document.getElementById("garden-trip-picker"),rows=picker?[].slice.call(picker.querySelectorAll("[data-trip]")):[];S("tripPicker",{prevented:openPrevented,open:window.__tripPickerOpen(),title:picker&&picker.querySelector(".dj-pick-panel>text").textContent,ids:rows.map(function(x){return x.getAttribute("data-trip");})});var openCtxPrevented=ctx(box),sceneMenu=document.querySelector(".scene-ctx"),escape=sceneMenu&&sceneMenu.querySelector("button"),escapeLabel=escape&&escape.querySelector("span:last-child");S("tripPickerContext",{prevented:openCtxPrevented,label:escapeLabel&&escapeLabel.textContent.trim()});if(escape)tap(escape);await sleep(20);S("tripPickerContextClosed",!window.__tripPickerOpen());ctx(box);var advances=0,realNext=window.__gardenDoNext;window.__gardenDoNext=function(){advances++;return true;};var escPrevented=!document.dispatchEvent(new KeyboardEvent("keydown",{key:"Escape",bubbles:true,cancelable:true})),escClosed=!window.__tripPickerOpen();ctx(box);var backPrevented=!document.dispatchEvent(new KeyboardEvent("keydown",{key:"Backspace",bubbles:true,cancelable:true})),backClosed=!window.__tripPickerOpen();window.__gardenDoNext=realNext;S("tripPickerKeys",{escPrevented:escPrevented,escClosed:escClosed,backPrevented:backPrevented,backClosed:backClosed,advances:advances});ctx(box);picker=document.getElementById("garden-trip-picker");var acid=picker&&picker.querySelector("[data-trip=acid]");if(acid)tap(acid);S("tripPick",{open:window.__tripPickerOpen(),state:window.__tripState()});',
  ' window.__resetDrugsbox();setLang("cs");ctx(box);var csMenu=document.querySelector(".mon-ctx");S("cs",{submit:submit.querySelector("text").textContent,unlock:csMenu&&csMenu.textContent.trim()});if(csMenu)document.body.click();window.__openPhoneAppHere("calendar",true);await sleep(40);var normal=[].find.call(document.querySelectorAll(".calx-day"),function(x){var n=x.querySelector(".calx-num");return n&&n.textContent==="17";});if(normal)tap(normal);S("normalCalendar",{date:window.date(),url:location.href,phone:!!document.querySelector(".phone-backdrop.show")});window.__calResetToday();if(window.__closePhoneModal)window.__closePhoneModal(true);',
...
check(s.tripPickerContext&&s.tripPickerContext.prevented&&s.tripPickerContext.label==="Escape"&&s.tripPickerContextClosed,"right-clicking open vitamins offers Escape and closes through it",s.tripPickerContext);
