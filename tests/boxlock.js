...
var HARNESS = [
  ...
  ' var openPrevented=ctx(box),picker=document.getElementById("garden-trip-picker"),rows=picker?[].slice.call(picker.querySelectorAll("[data-trip]")):[];S("tripPicker",{prevented:openPrevented,open:window.__tripPickerOpen(),title:picker&&picker.querySelector(".dj-pick-panel>text").textContent,ids:rows.map(function(x){return x.getAttribute("data-trip");}),labels:rows.map(function(x){var label=x.querySelector(".dj-pick-label"),street=x.querySelector(".dj-pick-street");return {formal:label&&label.firstElementChild&&label.firstElementChild.textContent,street:street&&street.textContent,italic:street&&getComputedStyle(street).fontStyle,color:street&&getComputedStyle(street).fill};})});var openCtxPrevented=ctx(box),sceneMenu=document.querySelector(".scene-ctx"),escape=sceneMenu&&sceneMenu.querySelector("button"),escapeLabel=escape&&escape.querySelector("span:last-child");S("tripPickerContext",{prevented:openCtxPrevented,label:escapeLabel&&escapeLabel.textContent.trim()});if(escape)tap(escape);await sleep(20);S("tripPickerContextClosed",!window.__tripPickerOpen());ctx(box);var advances=0,realNext=window.__gardenDoNext;window.__gardenDoNext=function(){advances++;return true;};var escPrevented=!document.dispatchEvent(new KeyboardEvent("keydown",{key:"Escape",bubbles:true,cancelable:true})),escClosed=!window.__tripPickerOpen();ctx(box);var backPrevented=!document.dispatchEvent(new KeyboardEvent("keydown",{key:"Backspace",bubbles:true,cancelable:true})),backClosed=!window.__tripPickerOpen();window.__gardenDoNext=realNext;S("tripPickerKeys",{escPrevented:escPrevented,escClosed:escClosed,backPrevented:backPrevented,backClosed:backClosed,advances:advances});ctx(box);picker=document.getElementById("garden-trip-picker");var acid=picker&&picker.querySelector("[data-trip=acid]");if(acid)tap(acid);S("tripPick",{open:window.__tripPickerOpen(),state:window.__tripState()});',
