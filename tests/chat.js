...
check(s.request.context && s.request.context.room === "office" && s.request.context.phase === 1 && Array.isArray(s.request.context.unlocked_rooms) && s.request.context.date && s.request.context.time && typeof s.request.context.session.seconds === "number" && s.request.context.actions_available.indexOf("room.go") >= 0 && s.request.context.actions_available.indexOf("app.open") >= 0 && /Call Prague/.test(s.request.context.instructions.office) && /not the game/.test(s.request.context.instructions.party_exit_hint) && s.request.context.current_hint !== s.request.context.current_hint_key, "request includes live state, the current instruction text, the full localized instruction catalog, and available action IDs", s.request.context);
