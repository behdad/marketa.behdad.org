...
var HARNESS = [
  ...
  ' window.fetch=function(url,opts){var n=captured.length;captured.push({url:String(url),method:opts&&opts.method,headers:opts&&opts.headers,body:opts&&opts.body});return new Promise(function(resolve){releases.push(function(){var replies=[{text:"سلام، من اینجا هستم.",action:null},{text:"دنبال می‌کنم.",action:null},{text:"You are in the office.",action:null},{text:"Let’s go.",action:{id:"room.go",args:{room:"garden"}}}];resolve(new Response(JSON.stringify({reply:JSON.stringify(replies[n]||{text:"Done.",action:null})}),{status:200,headers:{"Content-Type":"application/json"}}));});});};',
  ...
  ' var third=captured[2], payload3=third?JSON.parse(third.body):null; S("slashChat",{reply:/You are in the office/.test(dtOut.textContent),rawJson:/\\{\\s*"text"/.test(dtOut.textContent),history:window.__monitorChatHistory&&window.__monitorChatHistory(),request:payload3,dir:dtOut.lastElementChild&&dtOut.lastElementChild.getAttribute("dir")}); window.__toggleDropTerm();var actionPromise=window.__monitorChatAsk("Take me to the garden/party.");await sleep(30);if(releases[0])releases.shift()();await actionPromise;await sleep(40);S("action",{room:window.currentStageName,events:actions.slice(),history:window.__monitorChatHistory&&window.__monitorChatHistory()});window.fetch=oldFetch;',
