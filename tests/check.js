...
function checkAudioFadeCloseRace(file, script) {
  ...
  if (file === "rsvp.html") {
    ...
    if (ctxSites > 19) {
      issues.push("found " + ctxSites + " `new Ctx()` sites (expected <= 19: the beds (aquarium ambient + aquarium melody + workout + totoro rain + totoro woodland-melody + night-sky Satie projector + garden-party techno + garden-party waltz), the song pipeline, and getSfxCtx) — one-shot effects must use getSfxCtx()");
    }
  }
  if (issues.length === 0) {
