...
function harness(bbqOn) {
  ...
  return [
    ...
    ' report.after={room:window.__currentStageName,max:window.__maxUnlocked(),solved:window.__solvedRooms(),bbq:!!window.__loftControllers.bbq.status(),open:smoker.classList.contains("open"),smoking:smoker.classList.contains("smoking"),locked:dots.map(function(dot){return dot.classList.contains("locked");}),storedMax:JSON.parse(localStorage.getItem("loftCheckpoint:v1")).progress.maxUnlocked};',
