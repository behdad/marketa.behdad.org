...
var HARNESS = [
  ...
  'var saved={version:1,savedAt:Date.now(),progress:{room:"cuddly",maxUnlocked:4,phase2:true,party:false,daylight:true,bbq:false},puzzle:{cuddly:{door:false}},phone:null,album:null,systems:{openings:{open:wanted,grill:{uncovered:true,open:true}}}};',
  ...
  'var report={errors:[],steps:{}};function state(){var g=window.__coveredGrillState();return {open:wanted.map(function(id){return [id,document.getElementById(id).classList.contains("open")];}),nook:document.getElementById("cuddly-balcony-door").classList.contains("open"),grill:g,mouse:document.getElementById("cuddly-mouse").classList.contains("scurrying"),cat:document.getElementById("witchy-chest-cat-walk").classList.contains("out"),fairy:document.getElementById("witchy-chest-fairy").classList.contains("released"),roach:window.__kitchenRoachState()};}',
  ...
  ' window.__restoreCheckpointSystems({openings:{open:["not-a-door","kitchen-cabinet-2"],grill:{uncovered:false,open:true}}},"beforeStage");',
  ' var g=window.__coveredGrillState();report.steps.validated={one:document.getElementById("kitchen-cabinet-1").classList.contains("open"),two:document.getElementById("kitchen-cabinet-2").classList.contains("open"),bad:!!document.getElementById("not-a-door"),grill:g};',
...
var s = r.steps, restored = s.restored, persisted = s.persisted, validated = s.validated, interaction = s.interaction;
...
check(validated && !validated.one && validated.two && !validated.bad &&
  ...
  "restore accepts only known openings and cannot leave a covered grill lid open", validated);
