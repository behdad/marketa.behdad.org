...
function harness(saved, key, lock) {
  return [
    ...
    'var laptop=document.getElementById("office-laptop"),monitor=document.getElementById("office-monitor");report.steps.restored={room:window.__currentStageName,laptopOpen:laptop.classList.contains("open"),monitorOn:monitor.classList.contains("screen-on"),kettle:window.__kettleCheckpointState(),lock:window.__monitorLockState(),monitorLocked:monitor.classList.contains("monitor-locked"),monitorSaver:monitor.classList.contains("show-saver")};',
    'if(savedLock){window.__goToStage("office");await sleep(80);report.steps.office={lock:window.__monitorLockState(),monitorLocked:monitor.classList.contains("monitor-locked"),monitorSaver:monitor.classList.contains("show-saver"),saver:window.__monitorSaverState()};return;}',
    'window.__goToStage("kitchen");await sleep(1500);report.steps.steam={wisps:document.querySelectorAll(".steam-wisp").length,kettle:window.__kettleCheckpointState()};await sleep(6600);report.steps.parked={room:window.__currentStageName,laptopShow:laptop.classList.contains("show-saver"),laptop:window.__laptopSaverState(),monitorShow:monitor.classList.contains("show-saver"),monitor:window.__monitorSaverState()};window.__goToStage("office");await sleep(100);report.steps.office={laptopActive:laptop.classList.contains("saver-running"),laptop:window.__laptopSaverState(),monitor:window.__monitorSaverState()};}',
    '})();</script>'
  ].join("\n");
}
