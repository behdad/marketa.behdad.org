...
var HARNESS = [
  ...
  ' var cinemaProps=Array.prototype.slice.call(document.querySelectorAll("#cinema-room .cinema-prop")),propIds=cinemaProps.map(function(el){return el.id;}),cameraLeft=document.getElementById("cinema-camera-left"),cameraRight=document.getElementById("cinema-camera-right");click(cameraLeft);var cameraLeftOnly=[cameraLeft.classList.contains("reacting"),cameraRight.classList.contains("reacting")];cameraLeft.classList.remove("reacting");click(cameraRight);var cameraRightOnly=[cameraLeft.classList.contains("reacting"),cameraRight.classList.contains("reacting")];cameraRight.classList.remove("reacting");propIds.filter(function(id){return id!=="cinema-projector"&&id!=="cinema-sprinkler"&&id!=="cinema-window"&&id!=="cinema-camera-left"&&id!=="cinema-camera-right";}).forEach(function(id){click(document.getElementById(id));});var poufs=document.getElementById("office-nesting-poufs");report.steps.props={ids:propIds,tabs:cinemaProps.map(function(el){return el.getAttribute("tabindex");}),tabStops:document.querySelectorAll("#cinema-room [tabindex=\\"0\\"]").length,poufs:{inCinema:!!poufs.closest("#cinema-room-art"),inOffice:!!poufs.closest("#stage-office"),tabindex:poufs.getAttribute("tabindex")},reactions:window.__cinemaRoomState().reactions,cameraCluster:!!document.getElementById("cinema-cameras"),cameraCase:!!document.querySelector("#cinema-room-art rect[x=\\"648\\"][y=\\"218\\"]"),cameraFlashes:document.querySelectorAll(".cinema-camera-flash").length,cameraLeftOnly:cameraLeftOnly,cameraRightOnly:cameraRightOnly};',
