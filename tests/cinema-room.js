...
check(s.vimeoEnded && s.vimeoEnded.state.open && s.vimeoEnded.state.powered &&
  ...
  "a finished Vimeo film tears down its iframe and returns focus, controls, and audio to the selector",
  s.vimeoEnded);
...
check(/data\.event === "ready"\) tellVimeo\("addEventListener", "ended"\)/.test(source),
  "the Cinema subscribes to Vimeo's ended event after the embedded player is ready");
