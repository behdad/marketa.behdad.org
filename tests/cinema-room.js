...
var HARNESS = [
  ...
  ' var channel=window.__cuddlyProjector.channel(),before=window.__activeAudioBedCount(),ticket=document.getElementById("cuddly-cinema-ticket"),ticketBox=ticket.getBoundingClientRect(),screenBox=document.getElementById("cuddly-screen-bg").getBoundingClientRect();dblclick(ticket);await sleep(850);',
  ' report.steps.projectorOff={state:window.__cinemaRoomState(),frame:!!document.getElementById("cinema-player"),chooser:document.getElementById("cinema-chooser").hidden,off:!document.getElementById("cinema-screen-off").hidden};',
  ' var projector=document.getElementById("cinema-projector"),offScreen=document.getElementById("cinema-screen-off");click(offScreen);await sleep(40);report.steps.screenClickOn=window.__cinemaRoomState();click(projector);click(projector);await sleep(40);var clickOn=window.__cinemaRoomState();document.getElementById("hunt-playpause-btn").focus();key(" ");await sleep(40);var spaceOff=window.__cinemaRoomState();key("Enter");await sleep(40);report.steps.projectorControls={clickOn:clickOn,spaceOff:spaceOff,enterOn:window.__cinemaRoomState()};',
  ' var remotePower=document.getElementById("cinema-remote-google"),remoteRing=document.getElementById("cinema-remote-vava-ring"),remoteOk=document.getElementById("cinema-remote-vava-ok"),remotePad=document.getElementById("cinema-remote-pad"),vavaRemote=document.getElementById("cinema-remote-vava");function hitBox(el){var hit=el.querySelector(".cinema-hit");return ["x","y","width","height"].map(function(name){return +(hit.getAttribute(name)||0);});}click(remotePower);var remoteOff=window.__cinemaRoomState();click(remotePower);var remoteOn=window.__cinemaRoomState();click(remoteRing);var ringTactile=vavaRemote.classList.contains("remote-reacting"),remoteNext=window.__cinemaRoomState();click(remoteOk);var okTactile=vavaRemote.classList.contains("remote-reacting");await sleep(30);var remotePlay=window.__cinemaRoomState();click(remoteOk);await sleep(30);report.steps.remotes={off:remoteOff,on:remoteOn,next:remoteNext,play:remotePlay,back:window.__cinemaRoomState(),chooser:document.getElementById("cinema-chooser").hidden,current:Array.from(document.querySelectorAll(".cinema-film")).map(function(el){return el.classList.contains("remote-current");}),labels:[remotePower,remoteRing,remoteOk].map(function(el){return [el.getAttribute("role"),el.getAttribute("tabindex"),el.getAttribute("aria-label")];}),hits:{power:hitBox(remotePower),ring:hitBox(remoteRing),ok:hitBox(remoteOk),projector:hitBox(projector)},tactile:[ringTactile,okTactile],padBefore:!!(remotePad.compareDocumentPosition(remotePower)&Node.DOCUMENT_POSITION_FOLLOWING),vava:!!vavaRemote.querySelector("text")};',
  ...
  ' setLang("cs");report.steps.cs={title:document.getElementById("cinema-room-title").textContent,close:document.getElementById("cinema-room-close").getAttribute("aria-label"),window:cinemaWindow.getAttribute("aria-label"),neon:neon.getAttribute("aria-label"),remotes:[remotePower,remoteRing,remoteOk].map(function(el){return el.getAttribute("aria-label");})};setLang("en");',
  ...
  ' window.goToStage("cuddly");window.__cuddlyProjector.set("fire");window.__playSongAt(0);await sleep(100);window.__openCinemaRoom();report.steps.remoteResume={state:window.__cinemaRoomState(),current:document.querySelectorAll(".cinema-film.remote-current").length};click(film);await sleep(40);click(document.getElementById("cinema-room-close"));await sleep(900);report.steps.fastClose={open:window.__cinemaRoomState().open,song:window.__phoneMusicPlaying(),hidden:document.getElementById("cinema-room").hidden,viewport:document.querySelector(".hunt-viewport").classList.contains("cinema-room-open"),roster:[getComputedStyle(rosterToggle).visibility,getComputedStyle(roster).visibility,getComputedStyle(rosterBackdrop).visibility]};',
...
check(s.projectorOff && s.projectorOff.state.open && !s.projectorOff.state.powered &&
  !s.projectorOff.state.playing && !s.projectorOff.frame && s.projectorOff.chooser && s.projectorOff.off,
  "Cinema arrives with its physical projector off and no Vimeo surface", s.projectorOff);
...
check(s.projectorControls && s.projectorControls.clickOn.powered &&
  !s.projectorControls.spaceOff.powered && !s.projectorControls.spaceOff.playing &&
  s.projectorControls.enterOn.powered && !s.projectorControls.enterOn.playing,
  "Space and Enter toggle only projector power even when Play has focus", s.projectorControls);
check(s.remotes && !s.remotes.off.powered && s.remotes.on.powered &&
  ...
  s.remotes.labels.every(function(row){return row[0] === "button" && row[1] === null && row[2];}) &&
...
check(s.cs && s.cs.remotes && s.cs.remotes.every(function(label){return /[Oo]vlada/.test(label);}),
  "both physical remotes expose Czech control labels", s.cs && s.cs.remotes);
...
check(s.props && s.props.weights &&
  ...
  s.props.weights.hidden === "true" && s.props.weights.hit.join("|") === "88|250|57|35",
...
check(s.props && s.props.leftProjection.state.photobooth && s.props.leftProjection.state.powered &&
  ...
  s.props.leftProjection.back && s.props.leftProjection.tabs === 0 &&
  s.props.rightProjection.state.photobooth &&
  s.props.rightProjection.parent === "cinema-photobooth-surface" &&
  s.props.restored,
  "either camera projects the real Photobooth content without monitor chrome or new tab stops, and Back restores it", s.props);
