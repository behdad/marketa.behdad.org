...
var HARNESS = [
  ...
  ' var cinemaProps=Array.prototype.slice.call(document.querySelectorAll("#cinema-room .cinema-prop")),propIds=cinemaProps.map(function(el){return el.id;});propIds.filter(function(id){return id!=="cinema-projector"&&id!=="cinema-sprinkler"&&id!=="cinema-window";}).forEach(function(id){click(document.getElementById(id));});report.steps.props={ids:propIds,tabs:cinemaProps.map(function(el){return el.getAttribute("tabindex");}),tabStops:document.querySelectorAll("#cinema-room [tabindex=\\"0\\"]").length,reactions:window.__cinemaRoomState().reactions};',
