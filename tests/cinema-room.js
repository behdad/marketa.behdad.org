...
check(s.vimeoEnded && s.vimeoEnded.state.open && s.vimeoEnded.state.powered &&
  ...
  "Vimeo's trusted final timeupdate tears down its iframe and returns focus, controls, and audio to the selector",
  s.vimeoEnded);
...
check(/\["play", "pause", "ended", "timeupdate"\]\.forEach/.test(source) &&
  /event\.source !== frame\.contentWindow/.test(source),
  "the Cinema subscribes to Vimeo completion progress and accepts it only from the active iframe");
