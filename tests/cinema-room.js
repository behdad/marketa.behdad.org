...
var HARNESS = [
  ...
  ' var remotePower=document.getElementById("cinema-remote-google"),remoteRing=document.getElementById("cinema-remote-vava-ring"),remoteOk=document.getElementById("cinema-remote-vava-ok"),remotePad=document.getElementById("cinema-remote-pad");click(remotePower);var remoteOff=window.__cinemaRoomState();click(remotePower);var remoteOn=window.__cinemaRoomState();click(remoteRing);var remoteNext=window.__cinemaRoomState();click(remoteOk);await sleep(30);var remotePlay=window.__cinemaRoomState();click(remoteOk);await sleep(30);report.steps.remotes={off:remoteOff,on:remoteOn,next:remoteNext,play:remotePlay,back:window.__cinemaRoomState(),chooser:document.getElementById("cinema-chooser").hidden,current:Array.from(document.querySelectorAll(".cinema-film")).map(function(el){return el.classList.contains("remote-current");}),labels:[remotePower,remoteRing,remoteOk].map(function(el){return [el.getAttribute("role"),el.getAttribute("tabindex"),el.getAttribute("aria-label")];}),padBefore:!!(remotePad.compareDocumentPosition(remotePower)&Node.DOCUMENT_POSITION_FOLLOWING),vava:!!document.getElementById("cinema-remote-vava").querySelector("text")};',
