...
var HARNESS = [
  ...
  ' var close=document.getElementById("monitor-mines-close"),backMark=close.querySelector(".classics-back-arrow"),closeBg=close.querySelector(".classics-close-bg");S("game_close",{x:getComputedStyle(close.querySelector(".classics-close-x")).display,back:getComputedStyle(backMark).display,path:backMark.getAttribute("d"),fill:getComputedStyle(closeBg).fill,label:close.getAttribute("aria-label")});close.dispatchEvent(new MouseEvent("click",{bubbles:true}));S("back",{view:window.__classicsView(),stillOpen:mon.classList.contains("show-mines"),x:getComputedStyle(close.querySelector(".classics-close-x")).display,fill:getComputedStyle(closeBg).fill});',
  ...
  ' window.__solitaireLoadForTest({tableau:[[{suit:"s",rank:13}],[{suit:"h",rank:12}],[],[],[],[],[]]});var source=document.querySelector("[data-sol-source=tableau][data-sol-pile=\\"1\\"]"),target=document.querySelector("[data-sol-target=tableau][data-sol-pile=\\"0\\"]"),realPoint=document.elementFromPoint.bind(document);source.dispatchEvent(new PointerEvent("pointerdown",{bubbles:true,pointerId:91,button:0,clientX:20,clientY:20}));solitaireWrap=document.getElementById("monitor-solitaire-wrap");solitaireWrap.dispatchEvent(new PointerEvent("pointermove",{bubbles:true,pointerId:91,buttons:1,clientX:60,clientY:60}));var ghost=!!document.querySelector(".sol-drag-ghost");document.elementFromPoint=function(){return target;};solitaireWrap.dispatchEvent(new PointerEvent("pointerup",{bubbles:true,pointerId:91,button:0,clientX:60,clientY:60}));document.elementFromPoint=realPoint;st=window.__solitaireState();S("drag",{ghost:ghost,piles:st.tableau.map(function(x){return x.length;}),moves:st.moves});',
  ...
  ' window.__openMonitorApp("classics");window.__killMonitorClassics();S("killChooser",{chooser:mon.classList.contains("death-classics"),mine:mon.classList.contains("death-mines")});window.__deathFlashCleanup();',
...
check(s.game_close.x === "none" && s.game_close.back !== "none" &&
  s.game_close.path === "M375.4 157.9 L373.5 159.25 L375.4 160.6" &&
  s.game_close.fill === "rgb(47, 102, 83)" && s.back.fill === "rgb(200, 194, 180)" &&
  s.back.view === "chooser" && s.back.stillOpen && s.back.x !== "none",
  "the game-level top-right Back returns to the chooser; chooser retains Dismiss", { game: s.game_close, back: s.back });
...
check(s.drag.ghost && s.drag.piles[0] === 2 && s.drag.piles[1] === 0 && s.drag.moves === 1,
  "pointer dragging shows a ghost and lands a legal tableau move", s.drag);
