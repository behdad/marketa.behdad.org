...
var HARNESS = [
  ...
  ' await window.loft.app.open("console");for(var i=0;i<100&&!document.getElementById("office-monitor").classList.contains("show-console");i++)await new Promise(function(resolve){setTimeout(resolve,40);});report.surfaces.monitor=exercise(monitorIn,monitorOut);report.autocomplete.monitorHelp=tab(monitorIn,"help loft.wea");report.autocomplete.monitorJs=tab(monitorIn,"loft.wea");window.__closeMonitorConsole();window.__openDropTerm();report.surfaces.drop=exercise(dropIn,dropOut);report.autocomplete.dropHelp=tab(dropIn,"help loft.wea");report.autocomplete.dropJs=tab(dropIn,"loft.wea");',
