...
(function () {
  var m = html.match(/else if \(e\.key === "`"\) \{[\s\S]*?\n    \}/);
  if (!m) { bad("backtick handler present", "could not locate the `e.key === \"`\"` branch"); return; }
  var body = m[0];
...
(function () {
  ...
  var interpreterSrc = [
    ...
    "    if (c === 'party()') { consolePrint('party started'); return; }", // a representative command
  ...
  assert(newDropLines.indexOf("❯ party()") !== -1, "the typed line is echoed into the drop-down scrollback");
