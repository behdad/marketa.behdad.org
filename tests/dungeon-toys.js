...
var HARNESS = [
  ...
  'rect=chain.getBoundingClientRect();chain.dispatchEvent(new PointerEvent("pointerdown",{pointerId:18,clientX:rect.left+5,clientY:rect.top+5,bubbles:true,cancelable:true}));chain.dispatchEvent(new PointerEvent("pointermove",{pointerId:18,clientX:rect.left+5,clientY:rect.top+205,bubbles:true,cancelable:true}));report.steps.revealDrag=window.__princeDungeonState();chain.dispatchEvent(new PointerEvent("pointerup",{pointerId:18,clientX:rect.left+5,clientY:rect.top+205,bubbles:true,cancelable:true}));var wall=document.getElementById("prince-play-wall").getBoundingClientRect(),emblem=document.querySelector(".prince-play-emblem").getBoundingClientRect();report.steps.revealed={state:window.__princeDungeonState(),caption:window.__captionKey(),wallDisabled:document.getElementById("prince-play-wall").disabled,emblemX:(emblem.left+emblem.width/2-wall.left)/wall.width,emblemY:(emblem.top+emblem.height/2-wall.top)/wall.height};',
...
check(s.revealed && Math.abs(s.revealed.emblemX - .5) < .02 &&
  s.revealed.emblemY > .37 && s.revealed.emblemY < .45,
  "the revealed Play emblem sits in the upper-middle of the palace's center pane", s.revealed);
