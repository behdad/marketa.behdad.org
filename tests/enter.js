...
var HARNESS = [
  ...
  "    if(window.party)window.garden.set(true);await sleep(500);report.phaseTwo=!!window.__secondRound;",
  ...
  "    window.goToStage('garden');await sleep(300);var partyBefore=!!(window.party&&window.party.status());enter();await sleep(3400);var pac=window.__pacmanPresentation&&window.__pacmanPresentation();report.phaseTwoActions.partyToggled=partyBefore&&!window.party.status()&&!!(pac&&!pac.mode);",
  ...
  "    if(window.phone)window.phone.set(true);await sleep(180);enter();await sleep(120);report.phaseTwoActions.phoneOpenSafe=!!(window.phone&&window.phone.status())&&!!(window.__arcadeState&&!window.__arcadeState().active);if(window.phone)window.phone.set(false);await sleep(180);",
