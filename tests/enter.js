...
var HARNESS = [
  ...
  "    window.__goToStage('garden');var gg=document.getElementById('garden-guitar'),gu=document.getElementById('garden-ukulele'),gc=0,uc=0;if(gg)gg.addEventListener('click',function(){gc++;});if(gu)gu.addEventListener('click',function(){uc++;});enter();await sleep(700);enter();await sleep(700);report.gardenEnterGuitar=(gc===1&&uc===0);report.reached.garden=2+await pressUntil(1,10,700);report.after1=window.__currentStageIndex;",
  "    window.__goToStage('cuddly');report.reached.cuddly=await pressUntil(2,12,900);report.after2=window.__currentStageIndex;",
  "    window.__goToStage('office');await sleep(400);report.reached.office=await pressUntil(3,25,1500);await sleep(2500);report.after3=window.__currentStageIndex;",
  "    report.solvedFinalIdx=window.__currentStageIndex;",
  // ── Part 2: phase-two Enter launches each room's main activity; Escape closes it or stays inert ──
  "    if(window.__loftControllers.party)window.loft.garden.set(true);await sleep(500);report.phaseTwo=!!window.__secondRound;",
  "    window.__goToStage('kitchen');await sleep(300);enter();await sleep(200);report.phaseTwoActions.flairStart=!!(window.__flairState&&window.__flairState().active);key('Escape');await sleep(120);key('Backspace');await sleep(120);report.phaseTwoActions.flairClosed=!!(window.__flairState&&!window.__flairState().active);",
  "    window.__goToStage('garden');await sleep(300);var partyBefore=!!(window.__loftControllers.party&&window.__loftControllers.party.status());enter();await sleep(3400);var pac=window.__pacmanPresentation&&window.__pacmanPresentation();report.phaseTwoActions.partyToggled=partyBefore&&!window.__loftControllers.party.status()&&!!(pac&&!pac.mode);",
  "    if(window.__dismissActivePartyCoach)window.__dismissActivePartyCoach();await sleep(120);window.__goToStage('kitchen');if(window.__setDayNight)window.__setDayNight(false);await sleep(300);var coffeeBefore=window.__kitchenCoffeeNext&&window.__kitchenCoffeeNext(),coffeeClicks=0,coffeeEl=coffeeBefore&&document.getElementById(coffeeBefore);if(coffeeEl)coffeeEl.addEventListener('click',function(){coffeeClicks++;});enter();await sleep(120);var coffeeCaption=window.__captionKey&&window.__captionKey(),coffeeFlair=!!(window.__flairState&&window.__flairState().active);report.phaseTwoActions.dayCoffeeDetail={before:coffeeBefore,clicks:coffeeClicks,caption:coffeeCaption,flair:coffeeFlair,bar:!!(window.__barUpNow&&window.__barUpNow())};report.phaseTwoActions.dayCoffee=!!coffeeBefore&&coffeeClicks===1&&!coffeeFlair&&!report.phaseTwoActions.dayCoffeeDetail.bar&&coffeeCaption!=='kitchen_bar'&&coffeeCaption!=='kitchen_bar_night';",
  "    window.__goToStage('cuddly');await sleep(300);enter();await sleep(120);report.phaseTwoActions.octiStart=!!(window.__octiEscapeState&&window.__octiEscapeState().active);key('Escape');await sleep(120);report.phaseTwoActions.octiClosed=!!(window.__octiEscapeState&&!window.__octiEscapeState().active);",
  "    window.__goToStage('office');await sleep(300);enter();await sleep(120);report.phaseTwoActions.invadersStart=!!(window.__arcadeState&&window.__arcadeState().active);key('Escape');await sleep(120);report.phaseTwoActions.invadersClosed=!!(window.__arcadeState&&!window.__arcadeState().active);",
  ...
  "    if(window.__loftControllers.phone)window.__loftControllers.phone.set(true);await sleep(180);enter();await sleep(120);report.phaseTwoActions.phoneOpenSafe=!!(window.__loftControllers.phone&&window.__loftControllers.phone.status())&&!!(window.__arcadeState&&!window.__arcadeState().active);if(window.__loftControllers.phone)window.__loftControllers.phone.set(false);await sleep(180);",
  "    window.__goToStage('balcony');await sleep(300);enter();await sleep(120);report.phaseTwoActions.tetrisStart=!!(window.__balconyTetrisState&&window.__balconyTetrisState().active);key('Escape');await sleep(120);report.phaseTwoActions.tetrisClosed=!!(window.__balconyTetrisState&&!window.__balconyTetrisState().active);",
  "  }",
