...
var HARNESS = [
  ...
  ' window.__setMaxUnlocked(4);window.__setSecondRound(true,{releaseHeld:false});window.goToStage("balcony");await sleep(800);window.__openEntranceRoom();await sleep(90);report.steps.exterior=inspect(nav,time,row,vp,dots);',
  ...
  ' vp.dispatchEvent(new KeyboardEvent("keydown",{key:"`",code:"Backquote",bubbles:true,cancelable:true}));await sleep(60);var tab=document.getElementById("loft-console-hint"),fps=document.getElementById("dropterm-fps"),tools=document.getElementById("loft-console-tools"),tr=geom(tab),fr=geom(fps),tg=geom(tools),fg=geom(document.querySelector(".hunt-frame")),fs=geom(document.getElementById("hunt-fullscreen-btn"));report.steps.tools={keyboardOpen:!!window.__dropTermOpen(),tabOpen:tab.classList.contains("open"),toolsParent:tools.parentNode&&(tools.parentNode.id||tools.parentNode.className),tabParent:tab.parentNode&&tab.parentNode.id,fpsParent:fps.parentNode&&fps.parentNode.id,tabNorth:tr.bottom<=vr.top-5,fpsNorth:fr.bottom<=vr.top-5,tabX:Math.abs(tr.left-vr.left-3),fpsX:Math.abs(fr.left-vr.left-30),topControlY:Math.abs(tg.cy-fs.cy),frameLeft:fg.left,viewportLeft:vr.left,toolsLeft:tg.left,tabLeft:tr.left,tabInScene:vp.contains(tab),fpsInScene:vp.contains(fps),tabDisplay:getComputedStyle(tab).display,fpsDisplay:getComputedStyle(fps).display};',
...
function run(label, chromeFlags) {
  ...
  check(s.tools && s.tools.keyboardOpen && s.tools.tabOpen &&
    ...
    s.tools.tabNorth && s.tools.fpsNorth && s.tools.tabX < 1.1 && s.tools.fpsX < 1.1 && s.tools.topControlY < 4 &&
