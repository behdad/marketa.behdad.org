...
var HARNESS = [
  ...
  'setMotion(151,4);window.__entranceDriveControl("steerLeft",true);window.__entranceDriveControl("brake",true);step(1);report.steps.leftSpin={state:state(),spinning:car.classList.contains("spinning"),from:car.style.getPropertyValue("--porsche-spin-from"),to:car.style.getPropertyValue("--porsche-spin-to")};window.__entranceDriveControl("steerLeft",false);window.__entranceDriveControl("brake",false);await sleep(780);report.steps.leftSettled=state();setMotion(-151,4);window.__entranceDriveControl("steerRight",true);window.__entranceDriveControl("brake",true);step(1);report.steps.rightSpin={state:state(),spinning:car.classList.contains("spinning"),from:car.style.getPropertyValue("--porsche-spin-from"),to:car.style.getPropertyValue("--porsche-spin-to")};window.__entranceDriveControl("steerRight",false);window.__entranceDriveControl("brake",false);await sleep(780);report.steps.rightSettled=state();',
...
check(s.leftSpin && s.leftSpin.spinning && s.leftSpin.state.drive.spins === 1 &&
  ...
  "braking with steering just above 150 km/h starts a directional 180-degree yaw", s.leftSpin);
