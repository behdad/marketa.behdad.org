...
var HARNESS = [
  ...
  'report.steps.rpmMap={second100:window.__entranceDriveRpmForSpeed(100,2),sixth100:window.__entranceDriveRpmForSpeed(100,6),sixth263:window.__entranceDriveRpmForSpeed(263,6)};report.steps.benchmark100=benchmark100();report.steps.spatialMoved={engine:window.__entranceDriveSpatialAudio("engine"),music:window.__entranceDriveSpatialAudio("music")};',
  'setMotion(140,3);window.__entranceDriveControl("brake",true);step(1);window.__entranceDriveControl("brake",false);report.steps.brake=state();',
  ...
  'setMotion(214,4);window.__entranceDriveControl("steerLeft",true);window.__entranceDriveControl("brake",true);step(1);report.steps.leftSpin={state:state(),spinning:car.classList.contains("spinning"),from:car.style.getPropertyValue("--porsche-spin-from"),to:car.style.getPropertyValue("--porsche-spin-to")};window.__entranceDriveControl("steerLeft",false);window.__entranceDriveControl("brake",false);await sleep(780);report.steps.leftSettled=state();setMotion(-214,4);window.__entranceDriveControl("steerRight",true);window.__entranceDriveControl("brake",true);step(1);report.steps.rightSpin={state:state(),spinning:car.classList.contains("spinning"),from:car.style.getPropertyValue("--porsche-spin-from"),to:car.style.getPropertyValue("--porsche-spin-to")};window.__entranceDriveControl("steerRight",false);window.__entranceDriveControl("brake",false);await sleep(780);report.steps.rightSettled=state();',
...
check(s.started && s.started.state.car.engineOn && s.started.state.drive.hud &&
  ...
  s.started.controls.length === 13 && s.started.controls.every(function (row) { return row[2] === null; }),
...
check(s.brake && s.brake.drive.tireMarks >= 2 && s.brake.drive.speed < 140 &&
