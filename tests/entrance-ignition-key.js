...
var HARNESS = [
  ...
  'window.addEventListener("load",function(){setTimeout(async function(){try{Object.defineProperty(document,"hasFocus",{value:function(){return true;},configurable:true});window.__unlockAllRooms();window.__goToStage("balcony");var room=document.getElementById("entrance-room"),viewport=document.querySelector(".hunt-viewport"),strip=document.getElementById("loft-game-strip");room.style.transition="none";viewport.style.transition="none";strip.style.transition="none";window.__openEntranceRoom();await sleep(80);window.__openEntrancePorscheDriveHud();document.getElementById("entrance-drive-coach-dismiss").dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true}));await sleep(1000);window.scrollTo(0,0);report.off=snap();settleFace();window.__toggleEntrancePorscheEngine();await sleep(80);window.scrollTo(0,0);report.on=snap();window.__toggleEntrancePorscheEngine();await sleep(80);window.scrollTo(0,0);report.offAgain=snap();}catch(error){report.errors.push(String(error&&error.stack||error));}document.getElementById("__report").textContent=JSON.stringify(report);},220);});',
  '})();</script>'
].join("\n");
