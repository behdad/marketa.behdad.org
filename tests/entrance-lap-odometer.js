...
var HARNESS = [
  ...
  'window.__entranceDriveShift(6,true);window.__entranceDriveControl("throttle",true);for(var j=0;j<24;j++)window.__entranceDriveStep(80);window.__entranceDriveControl("throttle",false);await sleep(20);var beforeSave=state(),saved=window.__saveLoftCheckpoint(),raw=localStorage.getItem("loftCheckpoint:v1"),payload=raw&&JSON.parse(raw);window.__restoreCheckpointSystems(payload&&payload.systems,"afterStage");await sleep(40);report.steps={started:started,startedText:startedText,wrapped:wrapped,wrappedText:wrappedText,wrappedShape:wrappedShape,stopped:stopped,restarted:restarted,restartedText:restartedText,beforeSave:beforeSave,saved:saved,persisted:payload&&payload.systems&&payload.systems.entrance&&payload.systems.entrance.drive,restored:state()};',
