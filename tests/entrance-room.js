...
check(s.carInitial && !s.carInitial.state.roofOpen && !s.carInitial.state.doorOpen &&
  ...
  s.carInitial.roles.length === 9 && s.carInitial.roles.every(function(row){
    if (row[0] === "entrance-porsche-window") return row.slice(1).every(function(value){return value === null;});
...
check(s.carClosed && !s.carClosed.state.roofOpen && !s.carClosed.state.doorOpen &&
  !s.carClosed.state.windowOpen && !s.carClosed.state.frunkOpen && !s.carClosed.state.trunkOpen && !s.carClosed.state.engineOn &&
...
check(s.cs && s.cs.props && s.cs.props.length === 10 &&
  ...
  s.cs.car && s.cs.car.length === 9 &&
  s.cs.car.every(function(row,index){
    return s.carInitial.roles[index][0] === "entrance-porsche-window" ? row[0] === null && row[1] === null : row[0] && row[1] && row[0] === row[1];
  }),
  "existing Entrance facade and car labels and tooltips switch to Czech while the window adds no message", s.cs);
...
check((entrance.match(/class="entrance-car-control" role="button"/g) || []).length === 8 &&
  (entrance.match(/class="entrance-car-control"/g) || []).length === 9 &&
