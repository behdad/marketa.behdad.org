...
var HARNESS = [
  ...
  ' Object.defineProperty(document,"hasFocus",{value:function(){return true;},configurable:true});window.__unlockAllRooms();document.getElementById("hunt-fullscreen-area").dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true}));window.__markLowerRoomDiscovered();',
  ...
  ' if(window.__setDayNight)window.__setDayNight(false);if(window.__setWildfireSmoke)window.__setWildfireSmoke(1);window.__goToStage("balcony");await sleep(40);',
  ...
  ' window.__setLang("cs");report.steps.cs={label:room.getAttribute("aria-label"),floor:document.getElementById("hunt-floor-btn").getAttribute("aria-label"),props:props.map(function(prop){return [prop.getAttribute("aria-label"),prop.getAttribute("title")];}),car:carControls.map(function(control){return [control.getAttribute("aria-label"),control.getAttribute("title")];})};window.__setLang("en");',
  ...
  ' window.__goToStage("balcony");touchup(document.getElementById("balcony-background"));await sleep(20);touchup(document.getElementById("balcony-background"));await sleep(50);report.steps.touch=state();key("ArrowDown");await sleep(50);',
  ' key("ArrowLeft");await sleep(780);report.steps.left={source:state(),target:window.__bedroomRoomState(),room:window.__currentStageName};window.__goToStage("balcony");key("ArrowDown");await sleep(50);key("ArrowRight");await sleep(40);report.steps.right={source:state(),room:window.__currentStageName};',
  ' var targetDot=document.querySelectorAll(".hunt-dot")[1];targetDot.focus();click(targetDot);await sleep(780);report.steps.dot={source:state(),target:window.__princeState(),room:window.__currentStageName,focused:document.activeElement===targetDot};',
  ' window.__goToStage("balcony");key("ArrowDown");await sleep(50);click(document.getElementById("hunt-prev"));await sleep(780);report.steps.prev={source:state(),target:window.__bedroomRoomState(),room:window.__currentStageName};',
  ' window.__goToStage("balcony");key("ArrowDown");await sleep(50);click(document.getElementById("hunt-next"));await sleep(40);report.steps.next={source:state(),room:window.__currentStageName};',
  ' window.__goToStage("balcony");key("ArrowDown");await sleep(50);key("6");await sleep(780);report.steps.number={source:state(),target:window.__bathroomRoomState(),room:window.__currentStageName};',
...
check(s.interactive && !s.interactive.open && s.stageBackground && !s.stageBackground.open &&
  s.open && s.open.state.open &&
  s.open.room === "balcony" && s.open.viewport && !s.open.state.hidden,
  "Balcony props and the stage background stay upstairs while Down opens Entrance",
