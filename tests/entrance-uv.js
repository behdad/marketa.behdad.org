...
var HARNESS = [
  ...
  ' var strip=document.getElementById("loft-game-strip"),windows=Array.from(document.querySelectorAll("#entrance-room .entrance-prop[data-entrance-action=window]"));window.__setUvMode(true);windows.forEach(click);await sleep(40);var es=window.__entranceRoomState();report.steps.uv={caption:window.__captionKey(),flash:window.__flashCaptionState(),windowCount:windows.length,night:es.night,lights:es.windows,reactions:es.reactions,uvLive:strip.classList.contains("uv-mode")};',
