...
var HARNESS = [
  ...
  'function snap(){var room=document.getElementById("entrance-room"),rain=document.querySelector(".entrance-rain-weather"),snow=document.querySelector(".entrance-snow-flakes"),cloud=document.querySelector(".entrance-cloud-wash"),wet=document.querySelector(".entrance-wet-walk"),rainbow=document.getElementById("balcony-rainbow");return {classes:room.className.baseVal||room.className,rain:parseFloat(getComputedStyle(rain).opacity),snow:parseFloat(getComputedStyle(snow).opacity),cloud:parseFloat(getComputedStyle(cloud).opacity),wet:parseFloat(getComputedStyle(wet).opacity),rainbow:rainbow.classList.contains("after-rain"),nodes:document.querySelectorAll("#entrance-weather *").length,streaks:document.querySelectorAll(".entrance-rain-streak").length,caps:document.querySelectorAll("#entrance-architectural-snow path").length};}',
