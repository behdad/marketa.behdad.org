...
var HARNESS = [
  ...
  'function snap(){var room=document.getElementById("entrance-room"),rain=document.querySelector(".entrance-rain-weather"),snow=document.querySelector(".entrance-snow-flakes"),cloud=document.querySelector(".entrance-cloud-wash"),wet=document.querySelector(".entrance-wet-walk"),car=document.getElementById("entrance-porsche"),rainbow=document.getElementById("balcony-rainbow");function before(a,b){return !!(a.compareDocumentPosition(b)&Node.DOCUMENT_POSITION_FOLLOWING);}return {classes:room.className.baseVal||room.className,rain:parseFloat(getComputedStyle(rain).opacity),snow:parseFloat(getComputedStyle(snow).opacity),cloud:parseFloat(getComputedStyle(cloud).opacity),wet:parseFloat(getComputedStyle(wet).opacity),rainbow:rainbow.classList.contains("after-rain"),nodes:document.querySelectorAll("#entrance-weather *").length,streaks:document.querySelectorAll(".entrance-rain-streak").length,caps:document.querySelectorAll("#entrance-architectural-snow path").length,depth:{wetBeforeRain:before(wet,rain),rainBeforeCar:before(rain,car),carBeforeCloud:before(car,cloud)}};}',
...
check(s.rain && s.rain.depth && s.rain.depth.wetBeforeRain && s.rain.depth.rainBeforeCar &&
  s.rain.depth.carBeforeCloud,
  "rain paints over the wet street but behind every parked-car panel while the weather wash remains foreground",
  s.rain && s.rain.depth);
