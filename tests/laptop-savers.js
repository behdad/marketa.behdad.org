...
var HARNESS = [
  ...
  ' var laptop=document.getElementById("office-laptop");window.__restoreCheckpointSystems({laptop:{open:true,zoomed:false}},"beforeStage");window.__restoreCheckpointSystems({laptop:{open:true,zoomed:false}},"afterStage");await sleep(8100);S("continued",{open:laptop.classList.contains("open"),show:laptop.classList.contains("show-saver"),zoomed:window.__laptopZoomed(),state:window.__laptopSaverState(),caption:saverCaption()});window.__resetLaptop();laptop.classList.add("open");',
  ...
  ' window.__laptopZoomIn();await sleep(40);var sequence=[];function pushSaver(){var state=window.__laptopSaverState();state.caption=saverCaption();sequence.push(state);}window.__startLaptopSaver();await sleep(60);pushSaver();window.__cycleLaptopSaver();await sleep(60);pushSaver();window.__cycleLaptopSaver();await sleep(60);pushSaver();S("sequence",sequence);window.__monitorZoomOut();S("zoomOutCaption",saverCaption());',
...
check(r && r.steps.continued && r.steps.continued.open && r.steps.continued.show && !r.steps.continued.zoomed &&
  r.steps.continued.state.kind && r.steps.continued.state.cycling && !r.steps.continued.caption,
  "Continue rearms the open, unzoomed laptop's idle saver without captioning the room", r && r.steps.continued);
...
check(wake && wake.downPrevented && wake.clickPrevented && wake.first.awake &&
  wake.caption && wake.caption.key === "laptop_saver_caption_caps" && !wake.first.caption &&
...
check(sequence.length === 3 && order.length === 2 &&
  ...
  sequence.every(function (state) { return state.order.join("|") === order.join("|") && state.cycling &&
    state.caption && state.caption.owner === "laptop-saver" && state.caption.remaining > 3000 &&
    state.caption.key === "laptop_saver_caption_" + state.kind; }),
  "the once-shuffled order stays fixed across a complete wrap", sequence);
check(r && !r.steps.zoomOutCaption,
  "zooming out dismisses the laptop screensaver caption", r && r.steps.zoomOutCaption);
