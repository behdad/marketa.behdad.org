...
var HARNESS = [
  ...
  'function turnstileApi(token,state){return {render:function(_host,options){state.renders++;setTimeout(function(){options.callback(token);},0);return token;},execute:function(){state.executes++;},remove:function(){state.removes++;}};}',
  ...
  ' var p=caught(request()),s1=attempts.turnstile.shift();report.turnstile.pendingOwn=own.call(window,"turnstile");s1.dispatchEvent(new Event("error"));report.turnstile.error={result:await p,own:own.call(window,"turnstile"),attached:!!s1.parentNode};',
  ...
  ' var retrySettled=false,retry=request().then(function(value){retrySettled=true;return value;},function(error){retrySettled=true;throw error;}),current=attempts.turnstile.shift(),staleState={renders:0,executes:0,removes:0},liveState={renders:0,executes:0,removes:0};window.turnstile=turnstileApi("stale",staleState);hung.dispatchEvent(new Event("load"));await wait(50);var staleSettled=retrySettled;window.turnstile=turnstileApi("live",liveState);current.dispatchEvent(new Event("load"));report.turnstile.success={result:await caught(retry),staleSettled:staleSettled,own:own.call(window,"turnstile"),state:liveState};window.__resetMonitorChat();',
  ' p=caught(request());report.turnstile.reuse={result:await p,own:own.call(window,"turnstile"),queuedScripts:attempts.turnstile.length,state:liveState};window.__resetMonitorChat();',
...
check(result.turnstile.pendingOwn === false, "Turnstile starts without a placeholder global that triggers duplicate-load detection", result.turnstile.pendingOwn);
...
check(result.turnstile.success && result.turnstile.success.result.ok && result.turnstile.success.result.value === "live" && !result.turnstile.success.staleSettled && !result.turnstile.success.own, "a stale Turnstile load cannot settle the successful retry", result.turnstile.success);
check(result.turnstile.reuse && result.turnstile.reuse.result.ok && result.turnstile.reuse.result.value === "live" && !result.turnstile.reuse.own && result.turnstile.reuse.queuedScripts === 0 && result.turnstile.reuse.state.renders === 2 && result.turnstile.reuse.state.executes === 2 && result.turnstile.reuse.state.removes === 2, "Chat reset reuses the loaded Turnstile API without injecting its script again", result.turnstile.reuse);
