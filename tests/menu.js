...
var HARNESS = [
  ...
  "    await sleep(660); chatKill=window.__monitorChatKillState(); S('chat_kill_context_cleared', chatKill.stage==='cleared' && chatKill.system==='[context cleared]' && chatKill.systemVisible===true); S('chat_kill_caption', document.getElementById('hunt-caption').textContent==='Charlie left the chat. He still knows too much.');",
  ...
  "    await sleep(650); callKill=window.__monitorCallKillState(); S('call_kill_wave_cut', callKill.stage==='flatline' && callKill.bars===0 && callKill.wave<0.05 && callKill.flat>0.95); S('call_kill_caption', document.getElementById('hunt-caption').textContent==='The signal dropped. The feelings didn’t.');",
