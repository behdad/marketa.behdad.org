...
var HARNESS = [
  ...
  "    showApp('show-nowplaying'); var eqHit=document.querySelector('#monitor-manual-eq .meq-band rect[fill=transparent]'),eqr=eqHit&&eqHit.getBoundingClientRect();",
  "    if(eqHit&&eqr){eqHit.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,cancelable:true,pointerId:701,pointerType:'touch',button:0,buttons:1,clientX:eqr.left+eqr.width/2,clientY:eqr.top+1}));eqHit.dispatchEvent(new PointerEvent('pointerup',{bubbles:true,cancelable:true,pointerId:701,pointerType:'touch',button:0,clientX:eqr.left+eqr.width/2,clientY:eqr.top+1}));eqHit.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,cancelable:true,pointerId:702,pointerType:'touch',button:0,buttons:1,clientX:eqr.left+eqr.width/2,clientY:eqr.top+eqr.height/2}));await sleep(360);eqHit.dispatchEvent(new PointerEvent('pointercancel',{bubbles:true,cancelable:true,pointerId:702,pointerType:'touch',button:0,clientX:eqr.left+eqr.width/2,clientY:eqr.top+eqr.height/2}));var eqItems=monItems(),eqMenu=monMenu(),eqReset=eqMenu&&eqMenu.querySelector('.ctx-reset-eq');S('eq_touch_context',{items:eqItems,reset:!!eqReset,resetEnabled:!!eqReset&&!eqReset.disabled,kill:!!monKill()});if(eqReset)eqReset.click();S('eq_touch_reset',JSON.parse(localStorage.getItem('songEqBands')||'[]').every(function(v){return v===0;}));}",
