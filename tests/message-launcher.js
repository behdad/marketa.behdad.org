...
var HARNESS = [
  ...
  ' window.__runMsgAction("cue_mail");await sleep(30);if(window.phone)window.phone(false);await sleep(260);var badgeUse=badge.querySelector("use");S("read",{shown:badge.classList.contains("show"),idle:badge.classList.contains("idle"),icon:!!badgeUse&&badgeUse.getAttribute("href")==="#dicon-chat",label:badge.getAttribute("aria-label")});',
  ...
  ' if(window.__resetPhoneApps)window.__resetPhoneApps();if(window.phone)window.phone(false);await sleep(260);S("reset",{shown:badge.classList.contains("show"),idle:badge.classList.contains("idle")});',
  '}',
  '})();</script>'
].join("\n");
...
check(s.read.shown && s.read.idle && s.read.icon && /messages/i.test(s.read.label || ""), "a read thread keeps a neutral Messages launcher", s.read);
