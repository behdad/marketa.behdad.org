...
var HARNESS = [
  ...
  'async function run(){',
  ' window.__secondRound=true;window.__maxUnlocked=function(){return 4;};var calls=[],events=[];window.addEventListener("loft:statechange",function(event){events.push(event.detail);});window.goToStage=function(room){window.currentStageName=room;calls.push("room:"+room);};window.currentStageName="garden";',
...
check(s.failure.room === "kitchen" && s.failure.phone && s.failure.messages && /couldn.t do/i.test(s.failure.error || "") && s.failure.retry, "an unavailable authored action stays in Messages with a retryable failure", s.failure);
