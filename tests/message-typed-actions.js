...
var HARNESS = [
  ...
  'async function run(){',
  ' window.__secondRound=true;window.__gardenPartyOn=true;window.__maxUnlocked=function(){return 4;};var calls=[],events=[];window.addEventListener("loft:statechange",function(event){events.push(event.detail);});window.__goToStage=function(room){window.__currentStageName=room;calls.push("room:"+room);};window.__currentStageName="garden";',
  ' window.__deliverPhoneMessage("pouria");if(window.__hideMessageThumb)window.__hideMessageThumb();window.__openMessagesAt("pouria");await sleep(50);var row=document.querySelector(".pm-msg-row[data-message-id=pouria]"),button=row&&row.querySelector(".pm-msg-act");if(row)row.click();await sleep(20);S("row_click",{room:window.__currentStageName,phone:!!document.querySelector(".phone-backdrop.show"),read:!!(row&&!row.classList.contains("unread")),action:!!button,bubbleAction:!!(row&&row.querySelector(".pm-msg-bubble.actionable"))});if(button)button.click();await sleep(60);S("room_action",{room:window.__currentStageName,phone:!!document.querySelector(".phone-backdrop.show"),calls:calls.slice(),events:events.slice()});',
...
check(s.room_action.room === "kitchen" && s.room_action.calls.includes("room:kitchen"), "an authored room link executes through room.go", s.room_action);
...
check(s.failure.phone && s.failure.messages && s.failure.retry && !s.failure.used, "an unavailable authored action stays burgundy and retryable", s.failure);
