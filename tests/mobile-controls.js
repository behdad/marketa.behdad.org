...
var HARNESS = [
  ...
  ' var ids=["hunt-prev","hunt-next","hunt-restart-btn","hunt-fullscreen-btn","hunt-volume-btn","hunt-playpause-btn","hunt-skip-btn"],controls=ids.map(function(id){return rect(document.getElementById(id));}),dots=[].slice.call(document.querySelectorAll(".hunt-dot")).map(rect),all=controls.concat(dots),overlaps=[];',
...
check(s.overlaps.length === 0, "enlarged mobile targets do not overlap", s.overlaps);
