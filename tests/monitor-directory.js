...
var HARNESS = [
  ...
  ' setLang("cs");searchButton.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true}));await sleep(20);var csRows=results();S("czech",{rows:csRows,sorted:csRows.map(function(row){return row.label;}).slice().sort(function(a,b){return a.localeCompare(b);}),weather:csRows.filter(function(row){return row.id==="weather";})[0]&&csRows.filter(function(row){return row.id==="weather";})[0].label});',
  '}',
  '})();</script>'
].join("\n");
