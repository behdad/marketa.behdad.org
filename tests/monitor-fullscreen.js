...
var harness = [
  ...
  'window.goToStage("office");document.getElementById("office-pc-desk-trio").classList.add("on");mon.classList.add("here","screen-on","show-caps");window.__monitorZoomIn();await sleep(60);',
  'var before=screen.getBoundingClientRect(),searchHit=document.querySelector("#monitor-desk-search-button .desk-bar-hit").getBoundingClientRect(),fsHit=button.querySelector(".desk-bar-hit").getBoundingClientRect(),langHit=document.querySelector("#monitor-desk-language .desk-bar-hit").getBoundingClientRect();out.beforeRatio=before.width/before.height;out.visible=getComputedStyle(button).pointerEvents==="auto";out.taskbarGeometry={search:[searchHit.left,searchHit.right,searchHit.width,searchHit.height],language:[langHit.left,langHit.right],fullscreen:[fsHit.left,fsHit.right,fsHit.width,fsHit.height]};',
...
if (state && !state.error) {
  ...
  check(Math.abs(state.taskbarGeometry.search[2] - state.taskbarGeometry.search[3]) < 0.1 &&
      ...
      state.taskbarGeometry.fullscreen[0] > state.taskbarGeometry.language[1],
    "Search and Fullscreen use matching square taskbar cells, with Fullscreen right of EN/CZ",
  ...
  check(/Exit/i.test(state.enterLabel) && /Enter/i.test(state.exitLabel) &&
      /374\.65 155\.55/.test(state.enterMark),
    "the taskbar label and glyph reflect enter/exit state", state);
  ...
  check(state.noInnerControls && state.shootException,
    "one taskbar control stays monitor-local except for Shoot’s true-browser-fullscreen routing", state);
}
