...
var HARNESS = [
  ...
  ' ["fatality",["death-doom"],[]],["bsod",["death-linux"],["bsod-wrap"]],["photobooth",["photobooth"],["monitor-pb-videowrap"]],["photobooth-picker",["photobooth","picking"],["monitor-pb-picker-grid"]]',
  ' ];',
...
function run(label, chromeFlags) {
  ...
  check(result.photoboothPicker && result.photoboothPicker.picker && !result.photoboothPicker.physical &&
      !result.photoboothPicker.video && result.photoboothPicker.backing === "hidden" &&
      result.photoboothPicker.preview === "hidden" &&
      result.photoboothPicker.retakeFill === "#d3822f" && result.photoboothPicker.retakePaths === 2 &&
      Math.abs(result.photoboothPicker.retakeSize[0] - result.photoboothPicker.closeSize[0]) < 0.1 &&
      Math.abs(result.photoboothPicker.retakeSize[1] - result.photoboothPicker.closeSize[1]) < 0.1,
    "Photobooth picker exclusively fills the screen while its controls remain topmost", result.photoboothPicker);
