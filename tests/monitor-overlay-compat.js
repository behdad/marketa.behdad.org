...
var HARNESS = [
  ...
  ' clearSurface();monitor.classList.add("pb-confirm");await sleep(30);var confirm=document.getElementById("monitor-pb-confirm"),cr=confirm.getBoundingClientRect(),confirmHits=document.elementsFromPoint(cr.left+cr.width/2,cr.top+cr.height/2);report.photobooth={confirm:{overlay:window.__monitorHtmlOverlayState().active,dock:getComputedStyle(document.getElementById("monitor-desktop-dock")).visibility,visible:getComputedStyle(confirm).visibility,hit:confirmHits.some(function(el){return el===confirm||confirm.contains(el);})}};',
  ' monitor.classList.remove("pb-confirm");monitor.classList.add("pb-error");await sleep(30);var error=document.getElementById("monitor-pb-error");report.photobooth.error={overlay:window.__monitorHtmlOverlayState().active,dock:getComputedStyle(document.getElementById("monitor-desktop-dock")).visibility,visible:getComputedStyle(error).visibility};',
...
if (result) {
  ...
  check(!result.photobooth.confirm.overlay && result.photobooth.confirm.dock === "hidden" &&
      result.photobooth.confirm.visible === "visible" && result.photobooth.confirm.hit &&
      !result.photobooth.error.overlay && result.photobooth.error.dock === "hidden" &&
      result.photobooth.error.visible === "visible",
    "photobooth consent and error states exclusively own the visible screen", result.photobooth);
