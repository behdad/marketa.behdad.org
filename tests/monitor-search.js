...
check(s.touchArmed.armed && !s.touchArmed.active && s.touchArmed.pointer === "auto" && s.touchArmed.opacity > 0 && s.touchArmed.host === "hunt-fullscreen-area" && s.touchArmed.inputBox[0] >= 20 && s.touchArmed.inputBox[1] >= 16, "a painted native input is armed inside the fullscreen host before the first touch", s.touchArmed);
check(s.touchOpen.state.open && !s.touchOpen.state.query && s.touchOpen.prompt === "search apps…" && s.touchOpen.svgHidden === "hidden" && s.touchOpen.focused && s.touchOpen.defaultKept && s.touchOpen.aria === "Search apps" && s.touchOpen.pointer === "auto" && s.touchOpen.opacity === 1 && s.touchOpen.color !== "rgba(0, 0, 0, 0)" && s.touchOpen.inputBox[0] >= 32 && s.touchOpen.inputBox[1] >= 16, "a direct native-input touch opens a visibly editable finder without cancelling Android keyboard activation", s.touchOpen);
