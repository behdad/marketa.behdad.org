...
var HARNESS = [
  ...
  ' var searchButton=document.getElementById("monitor-desk-search-button"),touchInput=document.querySelector(".dock-search-keyboard"),searchClose=document.getElementById("monitor-dock-search-close");var armedRect=touchInput.getBoundingClientRect();S("touchArmed",{armed:touchInput.classList.contains("armed"),active:touchInput.classList.contains("active"),pointer:getComputedStyle(touchInput).pointerEvents,inputBox:[armedRect.width,armedRect.height]});var touchDown=new PointerEvent("pointerdown",{pointerType:"touch",bubbles:true,cancelable:true});touchInput.dispatchEvent(touchDown);await sleep(20);var searchSurface=document.getElementById("monitor-dock-search"),inputRect=touchInput.getBoundingClientRect();S("touchOpen",{state:window.__monitorDockSearch(),prompt:document.getElementById("monitor-dock-search-text").textContent,focused:document.activeElement===touchInput,defaultKept:!touchDown.defaultPrevented,aria:searchButton.getAttribute("aria-label"),pointer:getComputedStyle(searchSurface).pointerEvents,inputBox:[inputRect.width,inputRect.height]});touchInput.dispatchEvent(new PointerEvent("pointerdown",{pointerType:"touch",bubbles:true,cancelable:true}));S("touchRetains",{state:window.__monitorDockSearch(),focused:document.activeElement===touchInput});touchInput.value="wea";touchInput.dispatchEvent(new Event("input",{bubbles:true}));S("touchWeather",window.__monitorDockSearch());touchInput.value="clo";touchInput.dispatchEvent(new Event("input",{bubbles:true}));S("touchClock",window.__monitorDockSearch());touchInput.value="pac";touchInput.dispatchEvent(new Event("input",{bubbles:true}));S("touchMatch",window.__monitorDockSearch());touchInput.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:true,cancelable:true}));await sleep(30);S("touchLaunch",{pacman:mon.classList.contains("show-pacman"),state:window.__monitorDockSearch()});if(window.__closeTopMonitorApp)window.__closeTopMonitorApp();mon.classList.add("show-caps");await sleep(20);',
...
check(s.touchArmed.armed && !s.touchArmed.active && s.touchArmed.pointer === "auto" && s.touchArmed.inputBox[0] >= 20 && s.touchArmed.inputBox[1] >= 16, "the native input is already armed over the menu-bar icon before the first touch", s.touchArmed);
check(s.touchOpen.state.open && !s.touchOpen.state.query && s.touchOpen.prompt === "search apps…" && s.touchOpen.focused && s.touchOpen.defaultKept && s.touchOpen.aria === "Search apps" && s.touchOpen.pointer === "auto" && s.touchOpen.inputBox[0] >= 32 && s.touchOpen.inputBox[1] >= 16, "a direct native-input touch opens search without cancelling Android keyboard activation", s.touchOpen);
...
check(!s.emptyBackspace.open && !s.emptyBackspace.query, "Backspace dismisses an already-empty search before monitor navigation", s.emptyBackspace);
...
check(s.czechOpen.prompt === "hledat aplikace…" && s.czechOpen.aria === "Hledat aplikace", "the touch search prompt and accessible name follow Czech", s.czechOpen);
