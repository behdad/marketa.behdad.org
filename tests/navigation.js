...
var HARNESS = [
  ...
  'function dotState(){return [].slice.call(document.querySelectorAll(".hunt-dot")).map(function(d){return{label:d.getAttribute("aria-label"),disabled:d.getAttribute("aria-disabled"),selected:d.getAttribute("aria-selected")};});}',
  ...
  ' S("initial",nav("hunt-next"));S("dots_initial",{group:document.getElementById("hunt-dots").getAttribute("aria-label"),dots:dotState()});',
  ...
  ' setLang("cs");S("dots_czech",{group:document.getElementById("hunt-dots").getAttribute("aria-label"),dots:dotState()});',
...
check(s.dots_initial.group === "Rooms" && s.dots_initial.dots.map(function(d){return d.label;}).join("|") === "Kitchen|Garden|Cuddly-puddly|Office|Balcony" && s.dots_initial.dots.map(function(d){return d.disabled;}).join("|") === "false|true|true|true|true", "room dots expose English room names and accurate initial aria-disabled state", s.dots_initial);
check(!s.solved.locked && isWine(s.solved), "a normal solve immediately makes its arrow burgundy", s.solved);
check(s.dots_solved[1].disabled === "false" && s.dots_solved.slice(2).every(function(d){return d.disabled === "true";}), "unlocking a room immediately updates the dots' accessibility state", s.dots_solved);
...
check(s.dots_unlocked.every(function(d){return d.disabled === "false";}), "the all-rooms cheat exposes every dot as enabled", s.dots_unlocked);
check(s.dots_czech.group === "Místnosti" && s.dots_czech.dots.map(function(d){return d.label;}).join("|") === "Kuchyň|Zahrada|Cuddly-puddly|Pracovna|Balkon", "room-dot names and their tablist label follow the Czech UI", s.dots_czech);
