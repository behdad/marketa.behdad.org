...
var HARNESS = [
  ...
  'async function run(){',
  ' window.__goToStage("cuddly");await sleep(50);',
  ...
  ' window.__octiEscapeTest("target",2);target=2;var squishes=0,oldSquish=window.__playPlushSquishSound;window.__playPlushSquishSound=function(){squishes++;};var p1=pillows[0].getBoundingClientRect();document.getElementById("cuddly-marketa").dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true,clientX:p1.left+p1.width/2,clientY:p1.top+6}));await sleep(50);var missed=window.__octiEscapeState();window.__playPlushSquishSound=oldSquish;',
  ...
  ' window.__setCaption("explore_cuddly",true);window.__startOctiEscape();var escapePrevented=key(document,"Escape"),cancelled=window.__octiEscapeState();',
  ...
  ' window.__octiCheckpointRestore({inside:false});window.__startOctiEscape();window.__goToStage("kitchen");var left=window.__octiEscapeState();',
  ' S("room_leave_cancels",!left.active&&!left.inside&&!octi.classList.contains("in-chest")&&!stage.classList.contains("octi-escape-on"));',
  ' window.__goToStage("cuddly");window.__startOctiEscape();click(trunk);var trunked=window.__octiEscapeState();',
  ' S("trunk_keeps_toggle",!trunked.active&&trunked.inside&&octi.classList.contains("falling-in"));',
  ' window.__resetOctopusChest();var reset=window.__octiEscapeState(),resetJobs=window.__attentionScheduleState().jobs.filter(function(job){return /^minigame-octi/.test(job.owner);});',
