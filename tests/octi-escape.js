...
var HARNESS = [
  ...
  ' await sleep(1000);var input=document.createElement("input");document.body.appendChild(input);input.focus();var editablePrevented=key(input,String(target+1)),beforeEditable=window.__octiEscapeState().finds;input.remove();',
  ' S("editable_digits_untouched",!editablePrevented&&window.__octiEscapeState().finds===beforeEditable);',
  ' var numberPrevented=key(document,String(target+1));await sleep(60);var one=window.__octiEscapeState();',
  ' S("number_choice",numberPrevented&&one.active&&one.finds===1&&one.phase==="found");',
...
var checks = {
  ...
  editable_digits_untouched: "phone/editor-style inputs retain number keys while the game is active",
  number_choice: "1–3 chooses a cushion and is captured only during the game",
