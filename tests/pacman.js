#!/usr/bin/env node
// Focused lifecycle test for the ketamine-ghost Pac-Man unlock. The game stays out
// of discovery until the ghost is caught, retains a live maze across close/Continue,
...
var HARNESS = [
  ...
  'S("initial_locked",window.__pacmanUnlocked&&window.__pacmanUnlocked()===false&&dock&&dock.classList.contains("dock-app-locked")&&getComputedStyle(dock).display==="none");',
  ...
  'S("locked_undiscoverable",lockedSearch.query==="pac"&&!lockedSearch.match&&Array.isArray(lockedList)&&lockedList.indexOf("pacman")<0);',
  ...
  'wrap.querySelector(".pac-new").click();var fresh=window.__pacmanState();S("new_resets",fresh.status==="ready"&&fresh.score===0&&fresh.lives===3&&fresh.remaining===141);wrap.querySelector(".pac-btn[data-dir=left]").click();S("pad_plays",window.__pacmanState().status==="running"&&window.__pacmanState().running);',
  ...
  'monitor.classList.add("show-caps");await sleep(30);var desktopMenuPrevented=ctxAt(dock),desktopItems=menu()?[].map.call(menu().querySelectorAll("button"),function(b){return b.className;}):[];S("desktop_context",desktopMenuPrevented&&desktopItems.some(function(x){return /ctx-open/.test(x);})&&desktopItems.some(function(x){return /ctx-kill/.test(x);}));',
  'var openBtn=menu()&&menu().querySelector(".ctx-open");if(openBtn)openBtn.click();await sleep(60);var inAppPrevented=ctxAt(wrap),killBtn=menu()&&menu().querySelector(".ctx-kill");S("app_context",inAppPrevented&&!!killBtn&&!menu().querySelector(".ctx-restart"));if(killBtn)killBtn.click();',
  ...
  'var killed=window.__pacmanState(); S("kill_resets_not_relocks",killed.unlocked===true&&killed.status==="ready"&&killed.score===0&&!killed.dying&&!monitor.classList.contains("show-pacman")&&!dock.classList.contains("is-running"));',
