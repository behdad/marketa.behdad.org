...
try {
  ...
  for (const spec of cases) {
    ...
    const setup = await evaluate(`(async function () {
      ...
      return {ball:p,ballRect:ball.getBoundingClientRect().toJSON(),ballHit:hit&&!!hit.closest("#garden-disco-ball"),ballOpacity:getComputedStyle(ball).opacity,
    ...
    const map = await evaluate(`(async function(){
      ...
      return {kind:window.__partyCoachModalKind(),card:card.getBoundingClientRect().toJSON(),area:area.getBoundingClientRect().toJSON(),target:p,unrelated:np,
        ...
        messageQueued:!document.querySelector(".msg-badge-coach.show"),copy:overlay.querySelector(".party-bridge-room-copy").textContent,
    ...
    await evaluate("window.__closeDollhouse();window.__hideMessageThumb(true);window.__updateMsgBadge();window.__repeatMsgBadgeCoach()");
    await sleep(180);
    ...
    check(map.messageQueued && /All rooms are open/.test(map.copy),
      prefix + "Messages serializes behind the exploration-first copy", map);
