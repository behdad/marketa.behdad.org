...
var CANVAS_QUALITY_HARNESS = [
  ...
  'async function run(){window.__goToStage("office");var mon=document.getElementById("office-monitor"),julia=document.getElementById("monitor-saver-fractal"),pipes=document.getElementById("monitor-saver-pipes"),flower=document.getElementById("monitor-saver-flower"),flame=document.getElementById("cuddly-flame-img");mon.classList.add("here","screen-on","show-caps");[julia,pipes,flower].forEach(function(img){img.getBoundingClientRect=function(){return box(1240,420);};});flame.getBoundingClientRect=function(){return box(690,450);};report.steps.base={health:window.__frameHealthState(),julia:await saver(mon,"julia"),pipes:await saver(mon,"pipes"),flower:await saver(mon,"flower"),fire:window.__fireCanvasState()};window.__frameHealthFeed(60);window.__frameHealthFeed(60);window.__frameHealthFeed(60);var highHealth=window.__frameHealthState(),realHigh=window.__frameHealthHigh;window.__frameHealthHigh=function(){return true;};window.dispatchEvent(new CustomEvent("framehealthchange",{detail:highHealth}));report.steps.high={health:highHealth,julia:await saver(mon,"julia"),pipes:await saver(mon,"pipes"),flower:await saver(mon,"flower"),fire:window.__fireCanvasState()};window.__frameHealthFeed(45);var dropHealth=window.__frameHealthState();window.__frameHealthHigh=function(){return false;};window.dispatchEvent(new CustomEvent("framehealthchange",{detail:dropHealth}));var fireDrop=window.__fireCanvasState();report.steps.drop={health:dropHealth,julia:await saver(mon,"julia"),pipes:await saver(mon,"pipes"),flower:await saver(mon,"flower"),fire:fireDrop};window.__frameHealthHigh=realHigh;}',
  '})();</script>'
].join("\n");
