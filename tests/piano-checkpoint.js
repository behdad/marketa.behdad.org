...
var HARNESS = [
  ...
  'function snap(){var s=window.__projectorPianoState(),p=document.getElementById("cuddly-projector-piano");return {room:window.__currentStageName,channel:window.__cuddlyProjector.channel(),enabled:s.enabled,dismissed:s.dismissed,hidden:p.classList.contains("piano-dismissed"),captured:window.__captureCheckpointSystems()["projector-piano"]};}',
  'window.addEventListener("load",function(){setTimeout(async function(){try{',
  ' if(!sessionStorage.getItem("piano-checkpoint-seeded")){',
  '  window.__setMaxUnlocked(4);window.__setSecondRound(true,{releaseHeld:false});window.__goToStage("cuddly");window.__cuddlyProjector.set("stars");window.__dismissProjectorPiano();',
  ...
  ' report.steps.restored=snap();window.__goToStage("garden");window.__goToStage("cuddly");report.steps.returned=snap();',
