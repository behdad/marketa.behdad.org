...
var HARNESS = [
  ...
  ' var rp=document.getElementById("cuddly-poly-raffi"),cp=document.getElementById("cuddly-poly-chinnel"),rh=document.getElementById("cuddly-poly-raffi-head"),cb=document.getElementById("cuddly-poly-chinnel-body"),hosts=document.getElementById("cuddly-couple"),projector=document.getElementById("cuddly-wallscreen"),visitors=document.getElementById("cuddly-visitors-layer"),kidgames=document.getElementById("cuddly-kidgames"),dog=document.getElementById("cuddly-dog-pos"),k1=document.getElementById("cuddly-knife-1"),k2=document.getElementById("cuddly-knife-2"),hair=document.getElementById("cuddly-poly-chinnel-hair"),hairBox=hair.getBBox();',
  ' S("couch",{raffiFirst:!!(rp.compareDocumentPosition(cp)&Node.DOCUMENT_POSITION_FOLLOWING),chinnelRight:cp.transform.baseVal.consolidate().matrix.e>0,knife1:getComputedStyle(k1).visibility,knife2:getComputedStyle(k2).visibility,projectorBehindAll:!!(projector.compareDocumentPosition(hosts)&Node.DOCUMENT_POSITION_FOLLOWING)&&!!(projector.compareDocumentPosition(pair)&Node.DOCUMENT_POSITION_FOLLOWING),knivesAboveAll:!!(pair.compareDocumentPosition(k1)&Node.DOCUMENT_POSITION_FOLLOWING)&&!!(visitors.compareDocumentPosition(k1)&Node.DOCUMENT_POSITION_FOLLOWING)&&!!(kidgames.compareDocumentPosition(k2)&Node.DOCUMENT_POSITION_FOLLOWING)&&!!(dog.compareDocumentPosition(k2)&Node.DOCUMENT_POSITION_FOLLOWING),hair:{width:hairBox.width,height:hairBox.height,circles:hair.querySelectorAll("circle").length},headHit:getComputedStyle(rh).pointerEvents,bodyHit:getComputedStyle(cb).pointerEvents});',
  ...
  ' var blanket=document.getElementById("cuddly-blanket"),br=blanket.getBoundingClientRect(),bx=br.left+br.width/2,by=br.top+br.height/2;blanket.dispatchEvent(new PointerEvent("pointerdown",{bubbles:true,cancelable:true,pointerId:31,pointerType:"mouse",button:0,buttons:1,clientX:bx,clientY:by}));S("blanketLayer",{afterPair:!!(pair.compareDocumentPosition(blanket)&Node.DOCUMENT_POSITION_FOLLOWING),afterKnives:!!(k1.compareDocumentPosition(blanket)&Node.DOCUMENT_POSITION_FOLLOWING)&&!!(k2.compareDocumentPosition(blanket)&Node.DOCUMENT_POSITION_FOLLOWING),afterCreatures:!!(dog.compareDocumentPosition(blanket)&Node.DOCUMENT_POSITION_FOLLOWING)});blanket.dispatchEvent(new PointerEvent("pointerup",{bubbles:true,cancelable:true,pointerId:31,pointerType:"mouse",button:0,buttons:0,clientX:bx,clientY:by}));',
...
check(s.couch && s.couch.raffiFirst && s.couch.chinnelRight && s.couch.knife1 === "visible" &&
  s.couch.knife2 === "visible" && s.couch.projectorBehindAll && s.couch.knivesAboveAll &&
  s.couch.headHit !== "none" && s.couch.bodyHit !== "none",
  "the projector stays behind all four friends while both knives paint above every Cuddly figure", s.couch);
...
check(s.blanketLayer && s.blanketLayer.afterPair && s.blanketLayer.afterKnives && s.blanketLayer.afterCreatures,
  "a moved blanket becomes the final foreground above couples, creatures and knives", s.blanketLayer);
