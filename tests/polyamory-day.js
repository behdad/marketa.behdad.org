...
var HARNESS = [
  ...
  ' setLang("en");ph=document.querySelector(".calx-phone");d23=day(ph,23);d23.click();await sleep(80);',
  ' var pair=document.getElementById("cuddly-polyamory-pair");S("activate",{phone:!!document.querySelector(".phone-backdrop.show"),room:window.currentStageName,day:document.getElementById("loft-game-strip").classList.contains("polyamory-day"),opacity:getComputedStyle(pair).opacity,roster:(window.__whoIsHere("cuddly")||[]).map(function(p){return p.key;})});',
  ' party(true);await sleep(700);S("party",{opacity:getComputedStyle(pair).opacity,on:document.getElementById("loft-game-strip").classList.contains("party-on")});party(false);',
  ...
  ' S("models",{floor:!!document.querySelector(".g-chinnel")&&!!document.querySelector(".g-raffi"),bar:!!document.querySelector(".bc-chinnelraffi"),office:!!document.querySelector(".of-chinnelraffi"),balcony:!!document.getElementById("bh-chinnel")&&!!document.getElementById("bh-raffi")});',
...
var r = lib.runPageSync("rsvp.html", HARNESS, 2600, { patchRaf: true, seedRandom: true });
...
check(s.activate && !s.activate.phone && s.activate.room === "cuddly" && s.activate.day &&
  s.activate.opacity === "1" && s.activate.roster.indexOf("chinnel") >= 0 && s.activate.roster.indexOf("raffi") >= 0,
  "activating the day closes Calendar and gathers all four people in Cuddly", s.activate);
check(s.party && s.party.on && +s.party.opacity < 0.3, "the couch pair yields to party mode", s.party);
check(s.leave && !s.leave.day && s.leave.opacity === "0", "leaving November 23 tears down the couch scene", s.leave);
check(s.models && s.models.floor && s.models.bar && s.models.office && s.models.balcony,
  "both travel buddies have floor, balcony, bar and office art", s.models);
