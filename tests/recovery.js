...
var HARNESS = [
  ...
  ' var gate=document.getElementById("loft-recovery-gate"),buttons=gate&&gate.querySelectorAll(".loft-recovery-btn"),watch=document.querySelector(".watch-controls");S("gate",{shown:!!gate,clickMe:!!document.getElementById("click-me-overlay"),primary:buttons&&buttons[0].classList.contains("selected"),meta:gate&&gate.querySelector(".loft-recovery-meta").textContent,watchHidden:watch&&watch.hidden,watchAria:watch&&watch.getAttribute("aria-hidden"),watchDisplay:watch&&getComputedStyle(watch).display});',
  ...
  ' document.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:true}));S("continued",{gate:!!document.getElementById("loft-recovery-gate"),room:window.currentStageName,max:window.__maxUnlocked(),phase2:!!window.__secondRound,started:window.__gameStarted(),watchHidden:watch.hidden,watchAria:watch.hasAttribute("aria-hidden"),watchDisplay:getComputedStyle(watch).display});',
