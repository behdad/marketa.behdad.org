...
var HARNESS = [
  ...
  '<script>(function(){',
  'var saved={version:1,savedAt:Date.now()-120000,progress:{room:"office",maxUnlocked:4,phase2:true,party:true,daylight:true,bbq:false},puzzle:{cuddly:{door:true}},phone:null,album:null,systems:{pc:{powered:true},monitor:{surface:"monitor",screenOn:true,foreground:"mail",running:["mail","code"],zoomed:true},laptop:{open:true,zoomed:false},"phone-shell":{unlocked:true,open:true,app:"notes"},projector:{channel:"stars"},"kitchen-candle":{lit:true},"kitchen-kettle":{on:true},"kitchen-radio":{on:true},"garden-ac":{on:true},"balcony-lamp":{on:true},"office-faith":{symbol:"crescent"},"bar-neon":{label:"pamenar"}}};',
...
check(s.continued.partyRestored && s.continued.switchRestored && !s.continued.partyAfterP && !s.continued.switchAfterP,
  "one p press stops a restored party and keeps its switch synchronized", s.continued);
...
check(s.continued.devices && s.continued.devices.pc && s.continued.devices.monitor &&
  s.continued.devices.running.join(",") === "code,mail" && s.continued.devices.monitorZoom &&
  ...
  "Continue restores safe device shells and app identities without resuming active media or calls", s.continued.devices);
check(s.continued.devices && s.continued.devices.persisted &&
  s.continued.devices.persisted.pc.powered &&
  s.continued.devices.persisted.monitor.foreground === "mail" &&
...
check(startedOver && startedOver.translatedClickMe && startedOver.translatedClickMe.brand === "Den v podkroví" &&
  startedOver.translatedClickMe.word === "KLIKNI!" && startedOver.translatedClickMe.aria === "Den v podkroví. KLIKNI!",
  "changing language repaints the visible CLICK ME invitation and its accessible label", startedOver && startedOver.translatedClickMe);
