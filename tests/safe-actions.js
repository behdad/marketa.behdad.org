...
var HARNESS = [
  ...
  'window.__chatPhoneState=function(){return {open:false,call:null};};window.__madlaRing=function(){calls.push("madla-ring");return true;};window.__pragueRing=function(){calls.push("prague-ring");return true;};window.__maxUnlocked=function(){return 4;};window.__laptopCall=function(who){calls.push("video:"+who);return true;};',
  'var incoming=await window.loft.api.perform("call.incoming.trigger",{caller:"madla"},{source:"messages-chat"});var video=await window.loft.api.perform("call.video.start",{contact:"lubeck"},{source:"charlie-chat"});S("calls",{incoming:incoming,video:video,calls:calls.slice()});',
  'window.__arcadeState=function(){return {active:false};};window.__flairState=function(){return {active:false};};window.__ambientMaking=function(){return false;};window.__userMixing=function(){return false;};window.__bartenderDragging=function(){return false;};window.__makeCocktail=function(name,night){calls.push("cocktail:"+name+":"+night);return "ordered";};window.mixer=function(name){calls.push("mixer:"+name);return "mixing";};',
  'var cocktail=await window.loft.api.perform("bar.cocktail.make",{drink:"negroni"},{source:"messages-chat"});var mixer=await window.loft.api.perform("bar.mixer.start",{recipe:"yale"},{source:"charlie-chat"});var badDrink=await window.loft.api.perform("bar.cocktail.make",{drink:"beer"});S("bar",{cocktail:cocktail,mixer:mixer,bad:badDrink,calls:calls.slice()});',
  ...
  'window.chase=function(){calls.push("kids-chase");};window.butterfly=function(){calls.push("butterfly");};window.rainbow=function(){calls.push("rainbow");};',
...
check(s.calls.incoming.ok && s.calls.incoming.value.ringing && s.calls.video.ok && s.calls.video.value.contact === "lubeck" && s.calls.calls.includes("madla-ring") && s.calls.calls.includes("video:lueb"), "incoming and laptop calls use only authored contacts", s.calls);
...
check(s.scene.chase.ok && s.scene.butterfly.ok && s.scene.rainbow.ok && ["party.moment.start","call.incoming.trigger","call.video.start","bar.cocktail.make","bar.mixer.start","minigame.start","scene.activity.start"].every(function(id){return s.scene.caps.includes(id);}) && s.scene.events.some(function(e){return e.id==="scene.activity.start"&&e.source==="messages-chat";}), "safe scene actions are advertised and preserve action source", s.scene);
