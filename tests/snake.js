...
var HARNESS = [
  ...
  ' var dpad=document.getElementById("monitor-snake-dpad"),dpadButtons=[].slice.call(dpad.querySelectorAll(".snake-dpad-button")),dpadUp=dpad.querySelector("[data-snake-key=ArrowUp]"),dpadBefore=window.__snakeState();var dpadDownEvent=new PointerEvent("pointerdown",{bubbles:true,cancelable:true,pointerId:721,pointerType:"touch",button:0});dpadUp.dispatchEvent(dpadDownEvent);var dpadHeld=window.__snakeState();var dpadUpEvent=new PointerEvent("pointerup",{bubbles:true,cancelable:true,pointerId:721,pointerType:"touch",button:0});dpadUp.dispatchEvent(dpadUpEvent);var dpadReleased=window.__snakeState();S("dpad",{count:dpadButtons.length,keys:dpadButtons.map(function(b){return b.getAttribute("data-snake-key");}),right:dpadButtons.every(function(b){return Number(b.querySelector("rect").getAttribute("x"))>=344;}),before:dpadBefore,held:dpadHeld,released:dpadReleased,downPrevented:dpadDownEvent.defaultPrevented,upPrevented:dpadUpEvent.defaultPrevented,pressed:dpadUp.getAttribute("aria-pressed")});',
