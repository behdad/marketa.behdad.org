...
var HARNESS = [
  ...
  ' window.__openMonitorApp("snakes");await sleep(30);var first=document.querySelector("#monitor-snakes-wrap iframe");S("open",{open:mon.classList.contains("show-snakes"),frame:!!first,src:first&&first.getAttribute("src"),state:window.__snakesState()});var gutters=document.querySelectorAll("#monitor-snakes .monitor-runtime-side-hit");gutters[0].dispatchEvent(new MouseEvent("contextmenu",{bubbles:true,cancelable:true,clientX:250,clientY:250}));S("gutters",{count:gutters.length,menu:!!document.querySelector(".mon-ctx")});document.dispatchEvent(new KeyboardEvent("keydown",{key:"Escape",bubbles:true,cancelable:true}));',
