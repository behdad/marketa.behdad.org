...
var PROBE_HARNESS = [
  ...
  "  function clickOpeningGuideCard() { var e = document.querySelector('#opening-guide-coach .hunt-coach-copy'); if (!e) return false; e.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true })); return true; }",
  ...
  "    await sleep(1050); clickOpeningGuideCard();",
  "    ok('opening guide: the card body finishes after its pointer grace', !window.__openingGuideShowing() && has('kitchen-lamarzocco', 'invite-pulse'));",
  ...
  "",
  "    // Explicit special-day previews announce themselves in the quiet ribbon without taking over the",
  ...
  "    ok('special day: opening-guide arrow layer stays above the preview ribbon', !!dayBanner && parseInt(getComputedStyle(el('opening-guide-coach')).zIndex, 10) > parseInt(getComputedStyle(dayBanner).zIndex, 10));",
  "    if (guideCaption) guideCaption.classList.remove('intro-guide'); if (fsArea) fsArea.classList.remove('chrome-overlap');",
  "    ok('special day: the preview ribbon does not activate or navigate the event scene', !window.__gardenPartyOn && window.__currentStageIndex === 0, 'party=' + window.__gardenPartyOn + ' stage=' + window.__currentStageIndex);",
  "    history.replaceState(null, '', '?date=2031-10-31');",
  "    if (window.__applySeasonDate) window.__applySeasonDate();",
  "    if (window.__applyDateOccasion) window.__applyDateOccasion();",
  "    dayBanner = el('occasion-banner');",
  "    ok('special day: a named holiday reuses the preview occasion ribbon', !!dayBanner && /spooky season/i.test(dayBanner.textContent));",
  ...
  "    if (window.__applySeasonDate) window.__applySeasonDate();",
  "    if (window.__applyDateOccasion) window.__applyDateOccasion();",
  "    var realSpecialDayInfo=window.__specialDayBannerInfo;",
  "    history.replaceState(null, '', location.pathname);",
  "    window.__specialDayBannerInfo=function(){return { key: 'occ_banner_brunch' };};",
  "    if (window.__refreshOccasionBanner) window.__refreshOccasionBanner();",
  "    ok('special day: the real current date has no distracting occasion ribbon', !el('occasion-banner'));",
  "    window.__specialDayBannerInfo=realSpecialDayInfo;",
  "    history.replaceState(null, '', originalOccUrl);",
  "    if (window.__applySeasonDate) window.__applySeasonDate();",
  "    if (window.__applyDateOccasion) window.__applyDateOccasion();",
  "",
  ...
  "    var discoBall = el('garden-disco-ball'), discoOverlay = el('garden-disco-overlay'), discoSpin = el('garden-disco-overlay-spin');",
  "    ok('party controls: the wall-switch coach is absent and exploration is the first Party lesson', !el('party-switch-coach') && typeof window.__showPartySwitchCoach === 'undefined' && typeof window.__showPartyExplorationCoach === 'function');",
  "    ok('party controls: the phase-two disco ball is lit and animated while Party runs', parseFloat(getComputedStyle(discoBall).opacity) > .9 && parseFloat(getComputedStyle(discoOverlay).opacity) > .9 && getComputedStyle(discoSpin).animationName === 'disco-ball-spin');",
  "    gardenWallSwitch.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));",
  "    await sleep(40);",
  ...
  "    await sleep(4100);",
  "    ['firstDance','slowDance','toasts','groupPhoto','sparklers','cake','bdCake','bouquet','photoFreeze'].forEach(function (name) { if (window.__setPartyMomentState) window.__setPartyMomentState(name, false); });",
  "    await sleep(260);",
  "    ok('messages: first-Party reveal gate clears before the existing badge-coach cadence', window.__partyMessageRevealGateState && window.__partyMessageRevealGateState().complete);",
  ...
  "    var msgCoachCopy = msgCoach && msgCoach.querySelector('.msg-badge-coach-copy');",
  "    ok('messages: badge coach uses the shared large Fraunces treatment', !!msgCoachCopy && parseFloat(getComputedStyle(msgCoachCopy).fontSize) >= 26 && getComputedStyle(msgCoachCopy).fontFamily.indexOf('Fraunces') !== -1, msgCoachCopy && getComputedStyle(msgCoachCopy).cssText);",
  "    ok('messages: badge coach uses one coherent inline arrow path and a dismiss control', !!msgCoach && msgCoach.querySelectorAll('.hunt-coach-arrow').length === 1 && !!msgCoach.querySelector('.msg-badge-coach-dismiss'));",
