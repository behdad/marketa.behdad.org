...
var PROBE_HARNESS = [
  ...
  "",
  "    // The scene-level console tab starts hidden. Synthetic/autoplay input must not reveal it,",
  ...
  "    document.dispatchEvent(new KeyboardEvent('keydown', { key: '`', code: 'Backquote', bubbles: true, cancelable: true }));",
  "    var mask = el('garden-mask'), partyBeforeMask = !!(window.party && window.party());",
  "    if (mask) { mask.dispatchEvent(new MouseEvent('click', { bubbles: true })); mask.dispatchEvent(new MouseEvent('click', { bubbles: true })); }",
  "    ok('garden mask: two rapid clicks no longer toggle the party', !!mask && !!(window.party && window.party()) === partyBeforeMask && !(window.__dropTermOpen && window.__dropTermOpen()));",
  ...
  "    var familyKids = ['elisabeth', 'felix', 'patricia-son', 'patricia-daughter'];",
  ...
  "    ok('party kids: all seven runners are eligible after both families arrive', eligible.join('|') === allRunningKids.slice().sort().join('|'), 'eligible=' + eligible.join(','));",
  "    var parents = ['madla', 'robert', 'patricia'].map(function (name) { return document.querySelector('#garden-guests .g-' + name); });",
  ...
  "    var offsetRunners = ['elisabeth', 'felix', 'patricia-son', 'patricia-daughter', 'bahareh'];",
  ...
  "    var runnerAgeOrder = ['irene', 'robin', 'navid', 'patricia-son', 'elisabeth', 'felix', 'patricia-daughter'];",
  ...
  "    ok('party kids: runners are roughly 15% larger and step down with age', runnerScales.every(function (scale, i) { return scale > 0 && (!i || runnerScales[i - 1] > scale); }), 'scales=' + runnerScales.join(','));",
  "    var runnerFeet = { irene: 110, robin: 110, navid: 110, "patricia-son": 110, elisabeth: 106, felix: 104, "patricia-daughter": 110 };",
  ...
  "    ok('party kids: Robin and Navid periodically leave the dance floor for games', ['robin', 'navid'].every(function (name) { var fig = document.querySelector('#garden-guests .g-' + name); return fig && fig.classList.contains('off-at-games') && playEligible.indexOf(name) < 0; }) && playRoster.indexOf('Robin') < 0 && playRoster.indexOf('Navid') < 0);",
  "    if (window.goToStage) window.goToStage('cuddly'); await sleep(40);",
  "    var gameNames = window.__kidGamesNow ? window.__kidGamesNow().map(function (kid) { return kid.name; }) : [];",
  "    ok('party kids: the boys are present in the existing cuddly-room game', gameNames.indexOf('Robin') >= 0 && gameNames.indexOf('Navid') >= 0);",
  "    if (window.goToStage) window.goToStage('garden'); await sleep(40);",
  "    if (window.__summonGuests) window.__summonGuests();",
  "    if (window.__setPartyKidFormation) window.__setPartyKidFormation('play');",
  "    if (window.__startGardenChase) window.__startGardenChase(true);",
  "    var chasedKids = allRunningKids.filter(function (name) { var kid = el('garden-kid-' + name); return kid && kid.classList.contains('chasing'); });",
  "    ok('party kids: a forced chase runs exactly the seven children', chasedKids.length === 7, 'running=' + chasedKids.join(','));",
  "    ok('party kids: the explicit chase brings the godsons back from their game', document.getElementById('garden-guests').getAttribute('data-kid-formation') === 'free' && !document.querySelector('#garden-guests .g-robin').classList.contains('off-at-games') && !document.querySelector('#garden-guests .g-navid').classList.contains('off-at-games'));",
  ...
  "    ok(\"party kids: Who's here keeps the same identities during the floor-to-runner handoff\", rosterDuringChase.split('|').sort().join('|') === rosterBeforeChase.split('|').sort().join('|'), 'before=' + rosterBeforeChase + ' during=' + rosterDuringChase);",
  ...
  "    ok('party kids: the sleep announcement starts a persistent late-party floor bias', !!window.__kidsAsleep && !!window.__kidsLateParty && document.getElementById('garden-guests').getAttribute('data-kid-formation') === 'play' && lateFloorKids.every(function (name) { var fig = document.querySelector('#garden-guests .g-' + name); return fig && fig.classList.contains('off-at-games'); }));",
  "    if (window.__wakeKids) window.__wakeKids();",
  "    ok('party kids: waking does not immediately crowd every child back onto the dance floor', !window.__kidsAsleep && lateFloorKids.every(function (name) { return document.querySelector('#garden-guests .g-' + name).classList.contains('off-at-games'); }));",
  "    if (window.__setPartyKidFormation) window.__setPartyKidFormation('family');",
  "    ok('party kids: late-party children still return for a brief happy dance turn', lateFloorKids.every(function (name) { return !document.querySelector('#garden-guests .g-' + name).classList.contains('off-at-games'); }));",
  ...
  "    ok('party kids: ending the party clears its late-kid attendance bias', !window.__kidsLateParty && lateFloorKids.every(function (name) { return !document.querySelector('#garden-guests .g-' + name).classList.contains('off-at-games'); }));",
