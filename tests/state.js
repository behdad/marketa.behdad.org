...
var GATES_HARNESS = [
  ...
  "    ok('gates: balcony precipitation is cleared and paint-gated at the indoor room boundary', dropsBeforeLeave > 0 && document.querySelectorAll('.balc-drop').length === 0 && !document.getElementById('loft-game-strip').classList.contains('viewing-balcony'), 'before=' + dropsBeforeLeave);",
...
var PROBE_HARNESS = [
  ...
  "    var introOverlay = el('click-me-overlay'), introMachine = el('kitchen-lamarzocco');",
  ...
  "    await sleep(4200);",
  "    ok('opening guide: first clue remains until acknowledged', !!(window.__openingGuideShowing && window.__openingGuideShowing()) && has('hunt-caption', 'intro-guide'));",
  "    strip.dispatchEvent(new MouseEvent('pointerdown', { bubbles: true, cancelable: true }));",
  "    ok('opening guide: ordinary entry clears on acknowledgement', !(window.__openingGuideShowing && window.__openingGuideShowing()) && !has('hunt-caption', 'intro-guide'));",
  "    if (window.__showHuntIntro) window.__showHuntIntro();",
  ...
  "    await sleep(3000);",
  "    ok('opening guide: fullscreen caption still waits for acknowledgement', !!(window.__openingGuideShowing && window.__openingGuideShowing()) && has('hunt-caption', 'intro-guide'));",
  "    strip.dispatchEvent(new MouseEvent('pointerdown', { bubbles: true, cancelable: true }));",
  "    ok('opening guide: fullscreen caption ends on acknowledgement', !(window.__openingGuideShowing && window.__openingGuideShowing()) && !has('hunt-caption', 'intro-guide'));",
  ...
  "    if (overwaterPlant && window.waterSpecificPlant) {",
  "      window.waterSpecificPlant('garden-monstera', function(){return true;});",
  "      window.waterSpecificPlant('garden-monstera', function(){return true;});",
  ...
  "    await sleep(40);",
  "    ok('messages: p starts the party and phase two', !!window.__gardenPartyOn && !!window.__secondRound);",
  ...
  "    ok('messages: badge coach text is twice its former size', !!msgCoach && getComputedStyle(msgCoach).fontSize === '26px', msgCoach && getComputedStyle(msgCoach).fontSize);",
  ...
  "    msgCoach = document.querySelector('.msg-badge-coach');",
  "    ok('messages: the minute repeat shows again before the badge is clicked', !!msgCoach && msgCoach.classList.contains('show') && !!msgBadge && msgBadge.classList.contains('coached'));",
  ...
  "    ok('party kids: all eight assigned children are present in the Cuddly game', allRunningKids.every(function (name) { return gameNames.map(function (n) { return n.toLowerCase(); }).indexOf(name) >= 0; }));",
  ...
  "    ok(\"party kids: Who's here temporarily reports every Cuddly runner in the garden\", allRunningKids.every(function (name) { return rosterDuringChase.toLowerCase().indexOf(name) >= 0; }), 'before=' + rosterBeforeChase + ' during=' + rosterDuringChase);",
  "    if (window.__clearGardenChase) window.__clearGardenChase();",
