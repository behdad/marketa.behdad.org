...
var HARNESS = [
  ...
  'async function run(){',
  ' var oldMax=window.__maxUnlocked,oldPhone=window.__chatPhoneState,oldMessages=window.__chatMessagesSummary,oldArcade=window.__arcadeState,oldFlair=window.__flairState,oldContext=window.__chatContext,oldIndoor=window.__indoorTempState,oldOutdoor=window.__outdoorTempC;',
  ' window.__secondRound=true;window.currentStageName="office";window.__maxUnlocked=function(){return 3;};window.__chatPhoneState=function(){return {open:true,app:"messages",call:null};};window.__chatMessagesSummary=function(){return {total:7,unread:2,recent:[{id:"safe-id",sender:"Athena",outgoing:false,read:false}]};};',
  ...
  ' var smoker=document.getElementById("balcony-smoker");if(smoker)smoker.classList.add("smoking","open");window.__bbqPartySessionOn=true;',
  ...
  ' var q={progress:window.loft.api.query("game.progress"),busy:window.loft.api.query("game.busy"),rooms:window.loft.api.query("room.list"),bbq:window.loft.api.query("bbq.status"),projector:window.loft.api.query("projector.status"),currentApp:window.loft.api.query("app.current"),apps:window.loft.api.query("apps.list"),calls:window.loft.api.query("calls.status"),messages:window.loft.api.query("messages.summary"),games:window.loft.api.query("minigames.status"),environment:window.loft.api.query("scene.environment")};S("queries",q);',
  ' var calls=[];window.rain=function(on){window.__wxRain=!!on;calls.push("rain:"+on);};window.storm=function(on){window.__wxStorm=!!on;calls.push("storm:"+on);};window.overcast=function(on){window.__wxOvercast=!!on;calls.push("overcast:"+on);};',
  ...
  ' var auroraOn=false,twilightOn=false;window.aurora=function(on){auroraOn=!!on;calls.push("aurora:"+on);};window.twilight=function(on){if(arguments.length)twilightOn=!!on;return twilightOn;};window.__auroraStatus=function(){return {showing:auroraOn,kp:7};};window.__setTwilight=function(){return twilightOn;};',
  ...
  ' window.__maxUnlocked=oldMax;window.__chatPhoneState=oldPhone;window.__chatMessagesSummary=oldMessages;window.__arcadeState=oldArcade;window.__flairState=oldFlair;window.__chatContext=oldContext;window.__indoorTempState=oldIndoor;window.__outdoorTempC=oldOutdoor;',
  '}',
  '})();</script>'
].join("\n");
...
check(q.rooms.ok && q.rooms.value.length === 5 && q.rooms.value[3].current && q.rooms.value[3].unlocked && !q.rooms.value[3].solved && !q.rooms.value[4].unlocked, "room.list derives current, unlocked, and solved flags from the game frontier", q.rooms);
check(q.bbq.ok && q.bbq.value.on && q.bbq.value.smoking && q.bbq.value.open && q.bbq.value.party_split, "bbq.status reflects both smoker and party split state", q.bbq);
check(q.projector.ok && Object.keys(q.projector.value).sort().join(",") === "mode,on,room", "projector.status stays compact", q.projector);
...
check(q.calls.ok && q.calls.value.busy && q.calls.value.incoming && q.calls.value.laptop === "connecting", "calls.status summarizes active authored call surfaces", q.calls);
