...
var HARNESS = [
  ...
  "  var systemLinks=Array.from(sysinfo.querySelectorAll('.monitor-system-info-link'));",
  "  S('system_link_separators',Array.from(sysinfo.querySelectorAll('.monitor-system-info-link-separator')).map(function(el){return el.textContent}).join('')==='||');",
  "  var systemFooter=Array.from(sysinfo.querySelectorAll('.monitor-system-info-footer-piece')),systemSeps=Array.from(sysinfo.querySelectorAll('.monitor-system-info-link-separator')); S('system_footer_row',systemFooter.length===6&&systemFooter[5].classList.contains('monitor-system-info-recommendation')&&systemFooter.every(function(el){return +el.getAttribute('y')===193.5;})&&systemSeps.every(function(sep){var i=systemFooter.indexOf(sep),before=systemFooter[i-1].getBBox(),after=systemFooter[i+1].getBBox(),box=sep.getBBox();return box.x-(before.x+before.width)<1&&(after.x-(box.x+box.width))<1;}));",
  "  S('system_caption',window.__captionKey&&window.__captionKey()==='mon_system_info_caption');",
  "  S('system_open',sysinfo.classList.contains('open')&&/Browser/.test(sysinfo.textContent)&&!/Status/.test(sysinfo.textContent)&&!/No known issues/.test(sysinfo.textContent)&&/Graphics/.test(sysinfo.textContent)&&/Performance/.test(sysinfo.textContent)&&/Hardware/.test(sysinfo.textContent)&&/Best in the latest desktop Chrome\\./.test(sysinfo.textContent)&&systemLinks.length===3&&systemLinks.some(function(a){return /marketa\\.behdad\\.org$/.test(a.getAttribute('href')||'');})&&systemLinks.some(function(a){return /\\/issues$/.test(a.getAttribute('href')||'');})&&systemLinks.some(function(a){return /\\/issues\\/new$/.test(a.getAttribute('href')||'');}));",
  "  S('system_fit_en',inside(sysinfo.querySelector('.monitor-system-info-bg'),sysinfo.querySelectorAll('.monitor-system-info-title,.monitor-system-info-label,.monitor-system-info-value,.monitor-system-info-recommendation,.monitor-system-info-link text'))); window.__setLang('cs'); window.__openMonitorSystemInfo(); S('system_fit_cs',inside(sysinfo.querySelector('.monitor-system-info-bg'),sysinfo.querySelectorAll('.monitor-system-info-title,.monitor-system-info-label,.monitor-system-info-value,.monitor-system-info-recommendation,.monitor-system-info-link text'))); window.__setLang('en');",
...
ok("System keeps its links and recommendation on one compact row", s.system_footer_row === true);
