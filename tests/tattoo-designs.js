...
var harness = [
  ...
  ' var cells=[].slice.call(document.querySelectorAll(".tattoo-cell"));var thumb=document.querySelector(\'.tattoo-thumb[src$="ayushi.svg"]\');out.gallery={cells:cells.length,found:!!thumb,credit:thumb&&thumb.parentNode.querySelector(".tattoo-cap").textContent};',
...
if (state && !state.error) {
  check(state.gallery.cells === 9 && state.gallery.found && state.gallery.credit === "by Ayushi",
    "the gallery loads Ayushi's eighth built-in design with her credit", state.gallery);
