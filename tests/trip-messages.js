...
var HARNESS = [
  ...
  ' window.phone("messages");await sleep(80);var rows=[].slice.call(document.querySelectorAll(".pm-msg-row"));S("english",rows.map(function(r){return {from:r.querySelector(".pm-msg-from").textContent,body:r.querySelector(".pm-msg-text").textContent};}));',
...
check(en.length === 2 && en[0].from === "markéta" && en[1].from === "Alireza", "both trip cues draw independently from the six-sender pool", en);
check(en[0].body === "take a little trip? 🌈" && en[1].body === "Vitamin time? 💊", "both trip cues draw independently from the six-line pool", en);
check(cs[0].from === "markéta" && cs[1].from === "Alireza" && cs[0].body === "dáme si malý výlet? 🌈" && cs[1].body === "Čas na vitamíny? 💊", "frozen choices retain their identity across Czech translation", cs);
