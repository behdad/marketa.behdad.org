...
var HARNESS = [
  ...
  ' document.hasFocus=function(){return true;};window.__gameStarted=function(){return true;};window.loft.garden.set(false);window.__goToStage("cuddly");',
  ...
  ' window.__goToStage("garden");await sleep(50);hit.dispatchEvent(new MouseEvent("click",{bubbles:true,cancelable:true}));await sleep(40);S("roomGate",{room:window.__currentStageName,count:bunting.querySelectorAll(".cuddly-valentine-kiss-wrap").length});',
  ' window.__goToStage("cuddly");window.loft.garden.set(true);await sleep(80);S("party",{couple:couple.classList.contains("at-party"),visibility:getComputedStyle(couple).visibility});window.loft.garden.set(false);',
